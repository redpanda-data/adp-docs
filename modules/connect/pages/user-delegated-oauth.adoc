...
* An OAuth provider resource configured in the Agentic Data Plane UI under *OAuth providers*. The provider declares the upstream's `authorize_url`, `token_url`, supported scopes, and client credentials. See xref:connect:oauth-providers.adoc[Configure an OAuth Provider].
...
After consent, the user can see and revoke their connection under *Connections* in the Agentic Data Plane UI.
...
|The provider name on the server doesn't match an OAuth provider in the Agentic Data Plane UI. Check spelling and that the provider exists.
...
* xref:connect:create-server.adoc#configure-authentication[Service-account OAuth setup] for the shared-identity alternative.
