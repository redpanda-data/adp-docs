...
* For a managed MCP server, or to reuse a provider your organization already manages on a self-managed server: an OAuth provider resource configured on the *Outbound providers* tab of *Integrations setup*. The provider declares the upstream's `authorize_url`, `token_url`, supported scopes, and client credentials. See xref:connect:oauth-providers.adoc[Configure an OAuth Provider]. A self-managed server can instead set up OAuth automatically, described below, with no provider registered first.
...
. Configure how Redpanda gets an OAuth client for the upstream:
+
--
* For a *self-managed* (remote) server, pick a `Setup` mode:
+
** `Set up automatically` (the default): Redpanda registers an OAuth client and manages each user's tokens for you. When the server supports OAuth Dynamic Client Registration (DCR), there is nothing else to configure. When it doesn't, expand *Add a client ID* and enter the `Client ID` from the upstream's developer console, and, for a confidential client, a `Client Secret` (leave it empty for a public PKCE client). Redpanda shows the callback URL to add to the upstream app.
** `Attach existing provider`: select an OAuth provider you already registered (`UserOAuthAuth.provider_name`), or click *New* to create one without leaving the form.
* For a *managed* server, select the OAuth provider in the type's configuration (`UserOAuthAuth.provider_name`).
--
. Set the *required scopes* (`UserOAuthAuth.required_scopes`) the upstream API needs. Redpanda enforces these at consent time. On the automatic-setup path this is the optional *Set specific scopes* override; leave it empty to request whatever the server advertises.
...
+
// Verified 2026-07-30 against adp-ui remote-auth-fields.tsx on origin/main: the remote user-delegated form shows a Setup toggle ("Set up automatically" / "Attach existing provider"), an "Add a client ID" disclosure for servers without DCR (Client ID + optional Client Secret), and an optional "Set specific scopes" override. TokenInjection header_name/header_prefix remain API/CLI-level only.
. Save.
...
TIP: To configure user-delegated OAuth from the CLI, use `--user-oauth-provider` and `--user-oauth-scopes` on `rpk ai mcp-server create` or `rpk ai mcp-server update`. See xref:connect:create-server.adoc[Create an MCP Server].
