...
The page has two panes. A left rail groups the available OAuth providers into *Needs attention*, *Connected*, and *Available to connect*, with each row showing the provider's name, a status indicator, and a quick *Connect* or *Reconnect* action. Selecting a row opens a detail pane that shows the provider's requested scopes, its status (one of *Connected*, *Needs attention*, *Action required*, or *Not connected*), and *Connect*, *Reconnect*, and *Disconnect* actions. Click *Connect* to run the consent flow for that provider and store a token in the vault under your identity. Connections persist until you disconnect them. A connection whose access token can still be renewed from its stored refresh token stays *Connected*; one that can no longer be renewed moves to *Needs attention* so you can *Reconnect* it.
