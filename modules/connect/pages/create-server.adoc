...
* For user-delegated OAuth on a managed server, or to attach an existing provider on a self-managed server: an glossterm:OAuth provider[,OAuth provider] resource already configured. A self-managed server can instead set up OAuth automatically, with no provider registered first. See xref:connect:user-delegated-oauth.adoc[User-delegated OAuth].
...
|Each end-user authenticates against the upstream system with their own credentials, and Redpanda injects the user's token at call time. For a self-managed server, pick a `Setup` mode: `Set up automatically` lets Redpanda register an OAuth client for you (through Dynamic Client Registration where the server supports it, or from a client ID you supply where it doesn't), and `Attach existing provider` reuses an OAuth provider your organization already registered. A managed type that supports this mode attaches an OAuth provider in the type's configuration. The first time a user calls a tool that needs this server, Redpanda surfaces a consent prompt; Redpanda stores the resulting connection in the token vault, where it shows up under *My Connections*. See xref:connect:user-delegated-oauth.adoc[User-delegated OAuth] for the full flow.
|===
...
// Verified 2026-07-30 against adp-ui remote-auth-fields.tsx on origin/main: for a remote server the user-delegated form shows a Setup toggle ("Set up automatically" / "Attach existing provider"). Automatic setup registers a client through DCR, or takes a Client ID (and optional Client Secret) via the "Add a client ID" disclosure for a server without DCR, and offers an optional "Set specific scopes" override. TokenInjection header_name/header_prefix remain API/CLI-only. Screenshot still pending.
...
. Open the detail page. The *Overview* tab shows the `MCP Server URL`: the URL clients use to connect to the server. Copy it for use later.
. Open the *Activity* tab to review how agents have used this server. The *Recent calls* table lists recent tool calls across the agents attached to this server over the last 7 days, showing each call's time, tool, agent, caller, duration, and status. Click *Refresh* to reload the feed. If no traffic has run yet, the tab is empty.
...
// Tab names verified 2026-07-18 against adp-ui $name.index.page.tsx and $name.index.tsx (VALID_TABS) on main: Overview / Activity / Connection / Inspector. The Activity tab was added in cloudv2 PR #28227 and shows inferred recent tool calls; it is ungated. The Connection tab is the canonical home for client snippets (connect-client-guide + connection-tab components).
