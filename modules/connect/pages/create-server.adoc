...
. Open the *Activity* tab to review how agents have used this server. The *Recent calls* table lists high-confidence tool calls inferred from up to 50 recent conversations across the agents attached to this server over the last 7 days, showing each call's time, tool, agent, caller, duration, and status. Click *Refresh* to reload the feed. If no traffic has run yet, the tab shows an empty state instead.
