...
:page-aliases: mcp:create-server.adoc
...
* For user-delegated OAuth: an OAuth Provider resource already configured. See xref:connect:user-delegated-oauth.adoc[User-delegated OAuth].
...
For per-type fields, see the xref:connect:managed/managed-catalog.adoc[Managed catalog]: a reference of every managed MCP type Redpanda hosts, grouped by category, with a description and a link to its deep-dive page where one exists.
