...
// Tab names verified 2026-08-01 against adp-ui $name.index.page.tsx and $name.index.tsx (VALID_TABS) on main: Overview / Data Policies / Access / Activity / Connection / Inspector (the Data Policies tab's route value is `data-access`; its visible label is Data Policies). The Connection tab is the canonical home for both the MCP Server URL and the client snippets (connect-client-guide + connection-tab components); the redesigned Overview no longer prints the raw URL, and shows Attached agents, Server instructions, and Server metadata instead. The Activity tab shows inferred recent tool calls and is gated behind MCP_SERVER_ACTIVITY (the "ungated" claim in the 2026-07-18 version of this comment was stale). Data Policies is ungated and Preview-badged; Access is gated behind MCP_SERVER_ACCESS plus AGENT_PERMISSIONS. Both are documented separately.
