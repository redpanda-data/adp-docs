...
+
// TODO: xref the Agentic Data Plane secrets-management page once confirmed.
* For user-delegated OAuth on a managed server, or to attach an existing provider on a self-managed server: an glossterm:OAuth provider[,OAuth provider] resource already configured. See xref:connect:user-delegated-oauth.adoc[User-delegated OAuth].
...
+
// TODO: screenshot of the MCP Servers list page on the live environment.
...
// Verified 2026-07-30 against adp-ui on origin/main (remote-auth-fields.tsx, proxied-create-form.tsx): for a remote server the create form probes the URL and recommends automatic OAuth setup; the Setup toggle ("Set up automatically" / "Attach existing provider") sits behind "Use a different authentication method". The setup-mode detail lives on the user-delegated-oauth page, not in this table. TokenInjection header_name/header_prefix remain API/CLI-only. Screenshot still pending.
...
|JSON (default)
|Forwards results to the agent unchanged.

|TOON (token-optimized)
|Re-encodes tabular and nested-object results into a denser TOON representation that uses fewer tokens than JSON.

|JTON (token-optimized)
|Re-encodes tabular results into JTON, a compact superset of JSON.
|===
...
NOTE: Output format is a preview capability.

== Save and verify

. Click *Create server*. The server appears in the list with a *Type* badge: *Managed* or *Self-managed*.
. Open the detail page. The *Overview* tab summarizes the server, including its discovered tools, attached agents, and server metadata.
. If your organization has the *Activity* tab enabled, open it to review how agents have used this server. The *Recent calls* table lists recent tool calls across the agents attached to this server over the last 7 days, showing each call's time, tool, agent, caller, duration, and status. Click *Refresh* to reload the feed. If no traffic has run yet, the tab is empty.
. Open the *Connection* tab to get the `Server URL` that clients use to connect. Copy it for use later. The tab also gives you ready-to-paste connection snippets, pre-filled with this server's URL, for common MCP clients such as Claude Code, Claude Desktop, and Cursor, plus code examples in several languages. The code examples authenticate with the OAuth authorization-code flow and surface the OAuth client ID to use.
...
To mask, drop, or filter the data this server's tools return before an agent sees it, open the *Data Policies* tab. See xref:connect:data-policies.adoc[].

NOTE: Data policies are a preview capability.

// Tab names verified 2026-08-01 against adp-ui $name.index.page.tsx and $name.index.tsx (VALID_TABS) on main: Overview / Data Policies / Access / Activity / Connection / Inspector (the Data Policies tab's route value is `data-access`; its visible label is Data Policies). The Connection tab is the canonical home for both the server URL and the client snippets (connect-client-guide + connection-tab components); its field label is `Server URL` (not `MCP Server URL`, which is only the row-action label on the servers list), and it also exposes a `Code Mode URL` field for code-mode servers. The redesigned Overview no longer prints the raw URL, and instead shows the discovered tool list, Attached agents, Server instructions (when the server sends any), and Server metadata. The Activity tab shows inferred recent tool calls and is feature-gated (the "ungated" claim in the 2026-07-18 version of this comment was stale). Data Policies is ungated and Preview-badged; Access is feature-gated and additionally depends on the agent-permissions feature. Both are documented separately.
...
// TODO(rpk-ai): --auth-config does not exist on `rpk ai mcp-server create` today (verified against cloudv2/apps/rpai/internal/cmd/mcp/cmd.go on 2026-05-05). For non-convenience auth shapes, encode them inside `--managed-config` JSON. Re-add this row if a flag lands later.
...
// Source: cloudv2 proto/public/cloud/redpanda/api/adp/v1alpha1/mcp_server.proto (MCPServerService.ListMCPServers, ListMCPServersRequest.Filter.filter + order_by). The proto comment lists `display_name` as a filterable column, but the MCPServer resource has no display_name field, so it's omitted here. Orderable columns and the default order come from the storage table options (orderable: name, created_at; default_order_by: created_at desc). Request shapes are proto-accurate but were not exercised against a live cluster.
...
|The secret reference is case-sensitive and must be `UPPER_SNAKE_CASE` matching the proto regex `^[A-Z][A-Z0-9_]*$`. Confirm the secret exists in the Agentic Data Plane secret store and is spelled identically.
...
* xref:connect:data-policies.adoc[]
