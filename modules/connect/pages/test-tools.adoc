...
.The MCP Inspector follows one request from discovery through the protocol view
image::mcp-inspector-lifecycle.svg[alt="MCP Inspector flow. The Inspector negotiates the server's MCP version and capabilities, discovers tools, resources, and prompts, sends a selected tool call as JSON-RPC, and shows the formatted result beside the wire frames, timing, progress, and errors."]
...
// TODO: screenshot of a successful tool call (use a SQL `query` against a sample table on the live environment).
...
// TODO: screenshots of each error case once captured against the live environment.
