...
* The server's `MCP Server URL`. Copy it from the server's *Connection* tab.
...
|The server's `MCP Server URL` from AI Gateway. Format: `\https://aigw.<cluster-id>.clusters.rdpa.co/mcp/v1/<server-name>`.
...
|The MCP URL is wrong, or the Client ID + Client Secret don't match an OAuth Client. Confirm the `MCP Server URL` on the MCP server's *Connection* tab and the credentials on the OAuth Client's detail page.
