...
Register a client by hand when it supports neither client metadata documents nor dynamic client registration, when it needs a confidential `client_secret`, or when you want to pre-provision it before the first connection. For supported public clients, you can skip this manual flow. See <<client-metadata-documents,Let clients use metadata documents>> or <<self-register-with-dcr,Let clients self-register>>.
