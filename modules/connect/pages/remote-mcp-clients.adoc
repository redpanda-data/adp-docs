...
image::remote-mcp-client-authentication.svg[alt="Two-stage OAuth flow. An external MCP client authenticates inbound to AI Gateway through an OAuth client. AI Gateway invokes the MCP server, then authenticates outbound to the upstream system through an OAuth provider when user-delegated OAuth is configured."]
