...
:description: Connect external MCP clients (Claude Desktop, ChatGPT desktop, Gemini Apps) to MCP servers hosted in AI Gateway. Choose client metadata documents, dynamic client registration, or manual registration.
...
|OAuth Client
|Defines how an external chat app (Claude Desktop, ChatGPT, Gemini, Cursor) authenticates *against AI Gateway* on behalf of users. The client can use an HTTPS URL as its `client_id`, register dynamically, or use credentials from a manual registration. *This is what makes the chat-client integration possible.*
|===
...
== Let clients use metadata documents

Client ID Metadata Documents (CIMD) let a public OAuth client use an HTTPS URL as its `client_id`. The client hosts a JSON metadata document at that URL instead of creating a saved registration on AI Gateway. The document identifies the client and lists its redirect URIs.

When the client starts an authorization request, AI Gateway checks that the document names its own URL and includes the requested redirect URI. CIMD clients use the authorization-code flow with PKCE and do not use a shared client secret.

Use CIMD when the external MCP client supports it and you want clients to connect without an administrator creating or maintaining a registration. A client that uses a metadata document does not create an entry in the *Inbound clients* list.

=== Enable client metadata documents

. Open *Integrations setup* in the sidebar and select the *Inbound clients* tab.
. Select *Self-registration*.
. Turn on *Accept client metadata documents (CIMD)*.
. Under *Trusted document domains*, choose one of these options:
+
...
+
. Under *Allowed resources*, add the exact MCP URLs these clients may access, or allow any MCP server on the gateway.
. Click *Save settings*.

NOTE: If the settings report that client metadata documents are not active on the gateway, clients cannot use a URL as their client ID yet. Contact the gateway operator before testing the connection.
...
Register a client by hand when it supports neither client metadata documents nor dynamic client registration, when it needs a confidential `client_secret`, or when you want to pre-provision it before the first connection. For supported public clients, you can skip this manual flow. See <<client-metadata-documents,Let clients use metadata documents>> or <<self-register-with-dcr,Let clients self-register with DCR>>.

Create an OAuth Client to give the chat app the credentials it needs to authenticate against AI Gateway:

. Open *Integrations setup* in the sidebar and select the *Inbound clients* tab. The tab lists every external tool registered to request access tokens from this gateway, with its `Name`, `Grant Types`, `Status`, and `Scopes`. (The older `/oauth-clients` path still works but redirects here.)
...
// TODO: capture screenshots of the Add external tool catalog, the custom form, and the post-create detail page.

[[self-register-with-dcr]]
== Let clients self-register with DCR
...
TIP: You can also review and change self-registration from the UI: on the *Integrations setup* page, open the *Inbound clients* tab and select *Self-registration* to open its settings.
...
|`--allowed-resource`
|MCP server URL that every self-registered client may request tokens for. Repeatable. `*` allows any MCP server on the gateway.

|`--client-cap`
|Maximum number of concurrent self-registered clients.

|`--rate-per-hour`
|Maximum registrations accepted per hour.

|`--inactive-ttl-days`
|Days of inactivity before AI Gateway removes a self-registered client. `0` keeps clients indefinitely.
|===
...
|The server's `Server URL` from AI Gateway. Format: `\https://aigw.<cluster-id>.clusters.rdpa.co/mcp/v1/<server-name>`.
...
. Click *Connect* on the new connector row. Claude opens a browser tab pointed at AI Gateway's authorization endpoint. Sign in with your AI Gateway identity provider. After approval, the connector becomes invokable in any conversation.

// TODO: capture screenshots of the Add custom connector modal and the post-connect Connectors list against the live environment.
...
// TODO: confirm and document the ChatGPT, Gemini, Cursor, and Microsoft Copilot Studio menu paths once each integration is exercised end-to-end against the live environment. For Copilot Studio specifically: capture the Power Platform / agent designer route to "Add custom connector", the redirect URIs Microsoft publishes for the MCP connector flow, and any tenant-admin consent requirements. Copilot Studio has been named internally as a supported client, but no passing end-to-end test is confirmed; verify before promising the integration in customer-facing copy.
...
. Open *Integrations setup* in the sidebar, select the *Inbound clients* tab, and click the client whose tokens you want to revoke.
...
|The MCP URL is wrong, or the Client ID + Client Secret don't match an OAuth Client. Confirm the `Server URL` on the MCP server's *Connection* tab and the credentials on the OAuth Client's detail page.
