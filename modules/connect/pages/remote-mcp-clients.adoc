...
* You need programmatic, server-side tool invocation. See xref:gateway:connect-agent.adoc[Connect your agent] for SDK-based access.
...
. Sign in to ADP and open *OAuth Clients* in the sidebar. The page lists every external tool registered to request access tokens from this gateway, with its *Name*, *Grant Types*, *Status*, and *Scopes*.
...
On submit, AI Gateway mints the `client_id`. Confidential clients (those using a Client Secret auth method) also receive a `client_secret`, *shown only once*. Copy it immediately. The *OAuth Server Endpoints* panel shows the *Authorization Endpoint* and *Token/Refresh Endpoint* (under `/oauth/idp/`) and the `mcp` scope; paste these into the external app's OAuth configuration.
...
* A footer reminding the user that they can revoke access from their *Connections* page in the Agentic Data Plane UI.
+
...
* *List registered clients*: Open *OAuth Clients*. Each row shows the Name, Grant Types, Status, and Scopes.
* *Edit a client*: Change the display name, logo URI, redirect URIs, allowed MCP resources, grant types, or token endpoint auth method. The `client_id` is immutable; delete and recreate the client if you need a new one.
...
// TODO: confirm there is no in-place secret-rotation control (the edit page copy implies delete-and-recreate is the only path to a new secret). Verify against the OAuth client edit flow on adp-production.
...
. Open *OAuth Clients* and click the client whose tokens you want to revoke.
...
// TODO: confirm whether deletion of an upstream OAuth *Provider* (versus an OAuth *Client*) also triggers token revocation, or only Client deletion does. Verify against `apps/aigw/internal/oauthclient/` lifecycle handlers on cloudv2 main.
