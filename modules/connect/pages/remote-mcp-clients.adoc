...
Instead of registering every client by hand, you can let spec-conformant MCP clients register themselves. AI Gateway supports OAuth 2.0 Dynamic Client Registration (DCR, RFC 7591) and advertises a `registration_endpoint` in its authorization-server metadata at `/.well-known/oauth-authorization-server`. Clients that read this metadata (Claude Code, Cursor, ChatGPT, and other spec-conformant MCP clients) register on their first connection attempt, run authorization-code and PKCE, and obtain tokens without an admin provisioning a `client_id` or `client_secret` first.

Self-registration is off for each organization by default. Enable it before conformant clients can register.
