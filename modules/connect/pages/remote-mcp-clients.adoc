...
* The server's `MCP Server URL`. Copy it from the server's *Overview* tab.
...
|The MCP URL is wrong, or the Client ID + Client Secret don't match an OAuth Client. Confirm the `MCP Server URL` on the MCP server's *Overview* tab and the credentials on the OAuth Client's detail page.
