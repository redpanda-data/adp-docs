...
// Tools rail re-verified against apps/adp-ui/src/components/mcp-servers/tools-tab/tool-sidebar.tsx and tool-rail-card.tsx on origin/main, 2026-08-08: the rail now carries a "Filter tools" box (shown when the server exposes more than one tool) that matches on name, title, and description with a live "N of M" count, and a footer legend that counts the tools in each status. Long generated tool names wrap in full rather than truncating. Status-dot colors (green protected / yellow potential findings / gray no policy) are unchanged (lib/tool-status.ts STATUS_DOT).
...
When a server exposes many tools, use the *Filter tools* box to narrow the list by tool name, title, or description; the count next to *Tools* shows how many tools match.
