...
// Source: proto/public/cloud/redpanda/api/adp/v1alpha1/llm_provider.proto (anthropic_config.authorization_passthrough, api_key_ref); cloudv2 apps/rpai/internal/cmd/run/ and `rpk ai run <tool> --help`; apps/adp-ui/src/components/llm-providers. Verified against origin/main and the live environment on 2026-07-01. Bedrock support shipped 2026-07-02 and was verified end-to-end on the live environment at that time.
...
.`rpk ai run` routes coding-agent model traffic through AI Gateway
image::coding-agent-request-flow.svg[alt="Request flow from Claude Code or Codex through rpk ai run and AI Gateway to a compatible LLM provider. The coding agent receives model responses while provider credentials remain in the Agentic Data Plane secret store and usage flows to Cost and Usage."]
...
|The provider's *Status* is not *Active*. Enable it with `rpk ai llm-provider update <provider-name> --enabled`, or from the provider detail page.
...
|Run `rpk ai auth login` to sign in and select an Agentic Data Plane environment.
