...
:description: Make an agent render inline bar and line charts in the Inspector by emitting a chart code block that follows the current chart contract.
:page-topic-type: how-to
:personas: agent_builder
:learning-objective-1: Emit a chart code block that follows the current chart contract
...
An agent draws a chart by emitting a fenced code block tagged `chart` whose body is strict JSON that follows the current chart contract.
The Inspector renders the data with Redpanda Cloud's shared shadcn/Recharts components.
...
* A deployed agent in Redpanda Cloud. To create one, see xref:connect:create-agent.adoc[].
...
Add the convention to the agent's xref:connect:system-prompts.adoc[system prompt]: name the fence tag, state the strict JSON rules, list the supported types, and include 1 worked example.
...
|*Failed to render chart* appears in place of the chart
...
|*Unsupported chart type* appears
...
If the placeholder remains after the agent stops responding, instruct the agent to emit 1 complete `chart` block with a closing fence.
