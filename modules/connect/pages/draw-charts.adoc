...
.The Inspector turns a chart code block into three synchronized views
image::agent-chart-rendering.svg[alt="Chart rendering flow. An agent response contains a strict JSON chart code block. The Inspector validates the chart contract and produces Chart, Data, and Code views from the same configuration. Invalid configurations produce a visible inline error."]
