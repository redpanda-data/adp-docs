...
:learning-objective-3: Migrate an earlier chart prompt to the current chart contract
...
The Inspector renders the data with the Redpanda Cloud shadcn/Recharts chart components.
...
The current renderer uses shadcn/Recharts, not Chart.js.
Existing `bar` and `line` blocks written for the earlier Chart.js format still render.
The renderer reads `options.plugins.title.text` as the title, but ignores other legacy presentation options.
Other legacy chart types render as an error.
...
The shadcn/Recharts renderer supplies the visual presentation.
