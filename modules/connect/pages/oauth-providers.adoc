...
// Source: cloudv2 proto/public/cloud/redpanda/api/adp/v1alpha1/oauth_provider.proto (OAuthProviderService.ListOAuthProviders, ListOAuthProvidersRequest.Filter.filter + order_by). List permission `dataplane_aigateway_oauthprovider_list`; orderable columns and the default order come from the storage table options (orderable: name, display_name, created_at; default_order_by: created_at desc). Request shapes are proto-accurate but were not exercised against a live cluster.
