...
The two live as separate tabs — *Outbound providers* and *Inbound clients* — on the *Integrations setup* page, with separate API definitions, permissions, and lifecycles. To register or manage an OAuth client (including revoking its refresh tokens to force a re-sign-in), see xref:connect:remote-mcp-clients.adoc[Connect remote MCP clients to AI Gateway].
====
...
The *Outbound providers* tab on the *Integrations setup* page lists every provider registered in your organization. The list shows the following columns:
