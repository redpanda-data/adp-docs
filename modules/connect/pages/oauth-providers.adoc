...
+
* Pick a preset from the catalog to pre-fill the standard authorization and token endpoints and recommended scopes.
...
NOTE: This is the *outbound* direction: AI Gateway registers itself as a client of a remote MCP server's OAuth server. It is distinct from the inbound DCR that lets external MCP clients register themselves with AI Gateway. To let clients such as Claude Code or ChatGPT register with the gateway, see xref:connect:remote-mcp-clients.adoc[Connect remote MCP clients to AI Gateway].
