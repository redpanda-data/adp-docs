...
When you connect to a remote MCP server that authenticates users over OAuth, you can let AI Gateway detect the server's OAuth configuration instead of entering endpoints and credentials by hand. If the remote server supports OAuth Dynamic Client Registration (DCR), the gateway registers itself as an OAuth client automatically, so you have no endpoints or client credentials to paste.

NOTE: This is the _outbound_ direction: AI Gateway registers itself as a client of a remote MCP server's OAuth server. It is distinct from the inbound DCR that lets external MCP clients register themselves with AI Gateway. To let clients such as Claude Code or ChatGPT register with the gateway, see xref:connect:remote-mcp-clients.adoc[Connect remote MCP clients to AI Gateway].
...
|Server has no OAuth, or OAuth without DCR
