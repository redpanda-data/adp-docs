...
+
The catalog groups presets into categories you can filter with the category chips: *Identity & SSO*, *Code & Dev*, *Productivity*, *Files & Storage*, *Communication*, *CRM & Support*, *Data Platform*, *Observability*, *Infrastructure*, and *HR & Legal*. Selectable presets today are *GitHub*, *Google*, *Okta*, *Microsoft / Azure AD*, and *Slack*; more are rolling out. For any upstream without a preset, use *Custom Provider* and enter its endpoints manually.
+
// TODO: capture screenshots of the catalog picker. Update the selectable-preset list as more presets are enabled (UI allowlist: apps/adp-ui/src/lib/oauth-provider-presets.ts).
+
...
* *Token Exchange*: RFC 8693 server-to-server token exchange. Shown in the form but not yet functional (future release); the gateway will swap the user's identity-provider JWT for a provider-scoped token without browser interaction.
+
...
* xref:connect:managed/slack.adoc[Slack managed MCP]
* xref:connect:managed/jira.adoc[Jira managed MCP]
* xref:connect:managed/zendesk.adoc[Zendesk managed MCP]
* xref:connect:managed/workday.adoc[Workday managed MCP]
* xref:connect:managed/ironclad.adoc[Ironclad managed MCP]
* xref:connect:managed/ramp.adoc[Ramp managed MCP]
