...
* xref:gateway:configure-provider.adoc[AI Gateway configured] with at least one LLM provider enabled (this tutorial uses OpenAI).
...
Create a Remote MCP server with the following tools.
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/get_order_status.yaml[]
----
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/get_shipping_info.yaml[]
----
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/get_customer_history.yaml[]
----
====

. Click *Create*.

Wait for the server status to show *Running*. You now have focused tools the agent can orchestrate.
...
. On the *Basics* step, enter a *Display name* of `customer-support-agent` and a *Description* of `Helps customers track orders and shipping`. Click *Continue*.
. On the *Model* step, select your *LLM provider* and a *Model* with strong reasoning (such as GPT-5.2 or Claude Sonnet 4.5). Expand *Advanced* and set *Max iterations* to 15. Click *Continue*.
. On the *Behavior* step, enter this system prompt:
+
[source,text]
----
...
. On the *Tools* step, select `customer-support-tools` under *MCP servers*, then click *Continue*.
. On the *Subagents* step, leave the list empty and click *Continue*.
. On the *Review* step, click *Create agent*.

Wait for the agent status to show *Running*.

== Observe orchestration in action

Open the *Inspector* tab in ADP to interact with the agent.
