...
* Access to a Redpanda ADP environment. Redpanda provisions it for you when you purchase ADP.
* xref:get-started:gateway-quickstart.adoc[AI Gateway configured] with at least one LLM provider enabled (this tutorial uses OpenAI GPT-5.2 or Claude Sonnet 4.5 for reasoning).
...
. Click *Continue*, then on the *Review* step click *Create agent* to create the root agent with all four subagents.
...
* xref:connect:pipeline-integration-patterns.adoc[]
* xref:monitor:monitor-agents.adoc[]
* xref:connect:mcp-overview.adoc[]
