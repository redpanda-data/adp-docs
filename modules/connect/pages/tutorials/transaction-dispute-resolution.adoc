...
* xref:gateway:configure-provider.adoc[AI Gateway configured] with at least one LLM provider enabled (this tutorial uses OpenAI GPT-5.2 or Claude Sonnet 4.5 for reasoning).
* The xref:reference:rpk-install.adoc[Redpanda CLI (`rpk`)] installed (for testing the pipeline with sample data).
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/get_customer_account.yaml[]
----
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/get_transaction_details.yaml[]
----
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/get_transaction_history.yaml[]
----
====
...
[NOTE]
====
This tutorial uses XSmall resource tier for all MCP servers because the mock tools run lightweight Bloblang transformations. Production deployments with external API calls require larger tiers based on throughput needs. See xref:connect:mcp-overview.adoc[].
====
...
----
include::ROOT:example$mcp-tools/processors/calculate_fraud_score.yaml[]
----
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/get_risk_indicators.yaml[]
----
====
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/verify_merchant.yaml[]
----
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/get_merchant_category.yaml[]
----
====
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/log_audit_event.yaml[]
----
...
[,yaml]
----
include::ROOT:example$mcp-tools/processors/check_regulatory_requirements.yaml[]
----
====

. Click *Create*.

Wait for the server status to show *Running*. You now have an MCP server for each tool domain.

== Create the root agent with subagents

The root agent orchestrates sub-agents and makes final recommendations. You'll configure the root agent first, then add specialized sub-agents within the same form.

[IMPORTANT]
====
Sub-agents inherit the LLM provider, model, and max iterations from the root agent. This tutorial uses GPT-5 Mini and max iterations of 15 to optimize performance. Using slower models (GPT-5.2, Claude Sonnet 4.5) or high max iterations (50+) will cause sub-agents to execute slowly. Each sub-agent call could take 60-90 seconds instead of 10-15 seconds.
====

. Open *Agents* in the sidebar.
. Click *Create Agent*.
. Choose the managed option, *Redpanda manages it*, then click *Continue*.
. On the *Basics* step, enter a *Display name* of `dispute-resolution-agent` and a *Description* of `Orchestrates transaction dispute investigations`. Click *Continue*.
. On the *Model* step, select your *LLM provider* and the *Model* `GPT-5 Mini` (fast, cost-effective for structured workflows). Expand *Advanced* and set *Max iterations* to 15. Click *Continue*.
. On the *Behavior* step, enter the root agent system prompt, then click *Continue*:
+
[source,text]
----
include::ROOT:example$agents/dispute-root-agent-prompt.txt[]
----

. On the *Tools* step, don't select any *MCP servers*. The root agent uses the A2A protocol to call its sub-agents, not direct tools. Click *Continue*.

. On the *Subagents* step, click *Add subagent*.
...
[source,text]
----
include::ROOT:example$agents/account-agent-prompt.txt[]
----
...
[source,text]
----
include::ROOT:example$agents/fraud-agent-prompt.txt[]
----
...
[source,text]
----
include::ROOT:example$agents/merchant-agent-prompt.txt[]
----
...
[source,text]
----
include::ROOT:example$agents/compliance-agent-prompt.txt[]
----

. In the subagent's *MCP tools* section, select `compliance-tools`.

. Click *Continue*, then on the *Review* step click *Create agent* to create the root agent with all its subagents.
...
For detailed trace structure, see xref:monitor:concepts.adoc#agent-trace-hierarchy[Agent trace hierarchy].
...
. In ADP, go to *Connect* > *Secrets* (if not already there).
. Click *Create Secret*.
...
[,yaml,role="no-placeholders"]
----
include::ROOT:example$pipelines/dispute-pipeline.yaml[]
----
...
* Routes transactions with risk score ≥40 to the dispute-resolution-agent through A2A
...
. Open *Transcripts* in the sidebar.
...
For comprehensive troubleshooting guidance, see xref:monitor:troubleshoot-ai-agents.adoc[].
