...
[NOTE]
====
This tutorial uses XSmall resource tier for all MCP servers because the mock tools run lightweight Bloblang transformations. Production deployments with external API calls require larger tiers based on throughput needs. See xref:mcp:overview.adoc[].
====
...
For detailed trace structure, see xref:observability:concepts.adoc#agent-trace-hierarchy[Agent trace hierarchy].
...
* xref:agents:pipeline-integration-patterns.adoc[]
