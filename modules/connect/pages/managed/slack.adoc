...
:learning-objective-2: Walk through the consent flow and verify the connection on the My Connections page
...
. Slack redirects back to Agentic Data Plane. Your connection now appears under *My Connections*.
...
Once the server is created and at least one user has consented, you can point an agent at the `MCP Server URL` on the server's detail page. Each user calling the agent will trigger their own consent flow if they haven't connected yet.
