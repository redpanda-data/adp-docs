...
. Store the token in the Agentic Data Plane secret store under a name like `GRAFANA_SERVICE_ACCOUNT_TOKEN`.

Note your instance base URL and, if your instance is multi-org, the numeric org ID.

== Configure

Create a new Grafana MCP server in Agentic Data Plane:
...
[source,bash]
----
rpk ai mcp-server create --name acme-grafana --managed-config '{
