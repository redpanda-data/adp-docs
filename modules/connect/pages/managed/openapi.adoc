...
* Returns each successful result as structured output carrying the response's `status_code`, selected `headers`, and parsed `body`, so agents can read fields reliably instead of parsing raw text. A 4xx or 5xx response comes back as a tool error instead.
