...
* For *User OAuth* mode: a Zendesk OAuth client and an glossterm:OAuth provider[,OAuth Provider] configured in Redpanda Agentic Data Plane. See xref:connect:oauth-providers.adoc[Configure an OAuth Provider].
...
. Note the *agent email* the token will act as (the email of the user who created the token). The HTTP Basic authentication string the MCP builds is `base64(<email>/token:<api_token>)`. The `/token` literal is Zendesk's API-token quirk.
. Store the token in the Agentic Data Plane secret store under a name like `ZENDESK_API_TOKEN`.
...
. Register a matching OAuth Provider in Agentic Data Plane. See xref:connect:oauth-providers.adoc[Configure an OAuth Provider]. Use Zendesk's authorize and token endpoints.
...
// TODO: capture screenshots of the Zendesk create form on the live environment.
...
[source,bash]
----
rpk ai mcp-server create --name acme-zendesk --managed-config '{
...
[source,bash]
----
rpk ai mcp-server create --name acme-zendesk-oauth --managed-config '{
  "@type": "type.googleapis.com/redpanda.mcps.zendesk.v1.ZendeskMCPConfig",
  "subdomain": "acme",
  "user_oauth": {
