...
:page-beta: true
:description: Search and manage Zendesk Support tickets, users, and Help Center articles. Service-account API token or per-user OAuth, plus token-efficient response shaping for LLM agents.
...
Wraps the Zendesk REST API. The following auth modes are supported:
...
*Required scopes*: `read tickets:write hc:read` covers all tools. Drop `tickets:write` if the MCP only needs to read.
...
|`provider_name` (the Zendesk OAuth Provider you configured) and the minimum required scopes (`read`, `tickets:write`, `hc:read` covers all tools).
|===
+
. Click *Create*.
...
The Zendesk MCP exposes tools across tickets, users, organizations, and Help Center articles:
