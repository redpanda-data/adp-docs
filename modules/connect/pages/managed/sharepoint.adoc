...
[NOTE]
====
The SharePoint managed MCP server is in beta. Contact Redpanda support if you don't see SharePoint in the picker.
====

== What this MCP server does

The SharePoint managed type wraps the Microsoft Graph API (`https://graph.microsoft.com/v1.0`) and authenticates per-user through the gateway's OAuth token vault. It exposes tools across sites, document libraries, lists, and files:
...
. After registering, note the *Application (client) ID* and *Directory (tenant) ID* from the app's Overview page. You need both when you configure the glossterm:OAuth provider[,OAuth Provider].
