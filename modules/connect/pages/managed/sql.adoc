...
:description: Query and write to SQL databases (Postgres, MySQL, ClickHouse, MSSQL, SQLite, and more) through a managed MCP server hosted by Redpanda.
...
+
* `Driver`: One of `postgres`, `pgx`, `mysql`, `clickhouse`, `mssql`, `sqlite3`, `snowflake`, `trino`, or `databricks`.
