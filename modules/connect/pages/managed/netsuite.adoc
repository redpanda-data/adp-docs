...
* A NetSuite glossterm:OAuth provider[,OAuth Provider] in Redpanda Agentic Data Plane for the same account.
...
Register a NetSuite OAuth Provider in Agentic Data Plane with account-scoped endpoints:
...
For a non-UI path, use `rpk ai mcp-server create` with `--managed-config`. The top-level `--user-oauth-provider` and `--user-oauth-scopes` flags apply to self-managed servers only.
