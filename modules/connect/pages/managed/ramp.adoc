...
* An glossterm:OAuth provider[,OAuth Provider] configured in Redpanda ADP for Ramp. See xref:connect:oauth-providers.adoc[Configure an OAuth Provider].
...
Ramp authenticates per-user through the Ramp OAuth Provider you configured. The managed config carries only the environment; the `--user-oauth-provider` and `--user-oauth-scopes` flags apply to remote (`--url`) servers only.

// TODO: confirm how rpk associates the OAuth Provider with a managed Ramp server at create time. auth_config.user_oauth is a server-level field on the MCPServer that `rpk ai mcp create` does not currently set for managed backends.
