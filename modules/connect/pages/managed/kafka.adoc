...
+
// TODO: confirm reachability requirements (public bootstrap, peering, and so on) once the standalone Agentic Data Plane product surface ships.
* The cluster's bootstrap servers and SASL/TLS settings.
* For SCRAM or PLAIN: Secrets in the Redpanda Agentic Data Plane secret store for the username and password (`UPPER_SNAKE_CASE`, for example `KAFKA_SASL_USER` and `KAFKA_SASL_PASSWORD`).
...
// TODO: screenshot of the Kafka form filled in for a Redpanda Cloud cluster on the live environment.
...
After the Kafka server is created, point an agent at the `Server URL` on the server's *Connection* tab. The agent can then produce messages to topics through the exposed `produce` tool.
...
|Brokers aren't reachable from Agentic Data Plane egress. Confirm bootstrap addresses and any private-network requirements.
...
// TODO: gather more Kafka failure modes from the live environment walkthroughs.
