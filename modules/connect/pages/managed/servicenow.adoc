...
|*Auth method*
|*Basic auth*, *OAuth*, or *User-delegated OAuth*.

|*Username* (Basic auth only)
|ServiceNow service-account username (for example, `svc_redpanda`).

|*Password ref* (Basic auth only)
|Secret-store reference holding the service-account password. `UPPER_SNAKE_CASE`.

|*Client ID*, *Client secret ref*, *Token URL* (OAuth only)
|OAuth client ID, secret-store reference for the client secret, and the token endpoint (`https://<instance>.service-now.com/oauth_token.do`).

|*OAuth Provider* (User OAuth only)
|Pick the ServiceNow OAuth Provider you configured.
...
[source,bash]
----
rpk ai mcp create --name acme-servicenow-user \
  --managed-config '{
    "@type": "type.googleapis.com/redpanda.mcps.servicenow.v1.ServiceNowMCPConfig",
    "instance_url": "https://acme.service-now.com"
  }' \
  --user-oauth-provider servicenow-prod
----
======

Replace `<oauth-client-id>` with the client ID from your ServiceNow OAuth application registry.
...
|The table is not listed in `projections` with `queryable: true`. Add a projection entry for it.
