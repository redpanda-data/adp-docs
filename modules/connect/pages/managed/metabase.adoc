...
* One Redpanda Agentic Data Plane secret-store entry for the API key. Secret references must be `UPPER_SNAKE_CASE`, for example `METABASE_API_KEY`.
...
. Save the key in your Agentic Data Plane secret store under a name like `METABASE_API_KEY`.
...
Create a new Metabase MCP server in Agentic Data Plane:
...
// TODO: capture screenshots of the Metabase create form on the live environment.

=== Configure from the CLI

Run `rpk ai mcp-server create` with the Metabase managed-config payload:

[source,bash]
----
rpk ai mcp-server create --name metabase --managed-config '{
...
|API key is invalid or has been revoked. Mint a new key in *Settings > Authentication > API keys* and update the secret in the Agentic Data Plane secret store under the same name.
