...
:page-aliases: mcp:managed/metabase.adoc
...
* One ADP secret-store entry for the API key. Secret references must be `UPPER_SNAKE_CASE`, for example `METABASE_API_KEY`.
...
Create a new Metabase MCP server in the Agentic Data Plane UI:
...
The Metabase MCP exposes 11 tools. The agent's preferred path is the semantic-layer surface; the SQL-first surface is an escape hatch.
...
* xref:connect:managed/managed-catalog.adoc[Managed MCP Server Catalog]
* xref:connect:managed/sql.adoc[SQL managed MCP]
