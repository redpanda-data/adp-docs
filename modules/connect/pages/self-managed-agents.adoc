...
* At least one xref:gateway:configure-provider.adoc[LLM provider configured] in ADP. The agent calls the model through this provider.
* If the agent calls tools: One or more xref:connect:mcp-overview.adoc[MCP servers] registered in ADP.
...
. Click *Create agent*.
. Click *I host it myself*, so ADP registers the agent as an identity and leaves the runtime to you.
+
