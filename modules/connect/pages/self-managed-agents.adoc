...
// Source: cloudv2 `proto/public/cloud/redpanda/api/adp/v1alpha1/agent.proto` (Agent.agent_type oneof, OauthClient) cross-referenced against `apps/adp-ui/src/components/agents/agent-type-picker.tsx`, `self-managed-create-form.tsx`, `credentials-tab.tsx`, and `setup-tab.tsx` on origin/main, and verified on the live environment 2026-06-16.
...
* Access to create an agent, from an xref:control:access-policies.adoc[access policy] granting `Action::"Agent.create"`, or from the Admin role. See xref:control:permissions-reference.adoc#agent-management-permissions[Agent management permissions].
...
. Click *Create secret*.
. Optionally enter a *Name* to identify the secret in the list and in audit logs, for example, `production`. If you leave it blank, Agentic Data Plane assigns a default name such as `secret-1`.
