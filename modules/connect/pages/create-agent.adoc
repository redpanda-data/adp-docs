...
* *Authentication*: The agent card is public, but every other request to the endpoint requires an access token, sent as an `Authorization: Bearer` header. The caller also needs permission to invoke the agent. See xref:control:permissions-reference.adoc#a2a-runtime-permissions[A2A runtime permissions].
