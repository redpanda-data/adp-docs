...
// Re-verified against apps/adp-ui/src on cloudv2 origin/main, 2026-08-06: required fields carry a red asterisk (components/molecules/create-field.tsx); optional fields have no marker, and the canvas never shows "Required"/"Optional" text labels. The Tools search box renders only when the workspace has more than 8 MCP servers (managed-create/tool-list.tsx).
