...
// Source: `apps/adp-ui/src/components/molecules/markdown-editor.tsx`, `apps/adp-ui/src/lib/markdown-live-preview.ts`, `apps/adp-ui/src/components/agents/managed-create/instructions-field.tsx`, `managed-agent-canvas.tsx`, and `prompt-starters.ts` on cloudv2 main, verified 2026-08-05. The shared Markdown editor now has Live (default), Markdown, and Preview modes, replacing the earlier Write/Preview tabs. Live decorates the CodeMirror surface without changing the stored source; Markdown shows the exact source; Preview renders the value. The formatting toolbar and the import control render in both editing modes and are hidden only in Preview. The same editor is used for subagent instructions.
// A follow-up polish commit in the same landing removed the Templates section action: the prompt starter cards (Support agent, Data analyst, Coding assistant, Incident responder, Research coordinator) are now always visible above the editor and each one seeds only the system prompt. Do not reintroduce a Templates control here. The canvas has no visible "System prompt" field label either; the section is titled Instructions and the expanded dialog is titled "Edit instructions". "System prompt" is a visible label only on the agent detail Overview and in the subagent drawer.
