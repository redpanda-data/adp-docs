...
+
--
Use the OIDC `client_credentials` grant to mint an access token, then hand the token to Claude Code through `ANTHROPIC_AUTH_TOKEN`. This is the same flow xref:gateway:connect-agent.adoc[Connect your app to an LLM provider] documents for SDK clients; the only thing different here is how Claude Code reads the token.
