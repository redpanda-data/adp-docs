...
For implementation details, see xref:connect:mcp-overview.adoc[].
...
For implementation details, see xref:connect:pipeline-integration-patterns.adoc[].
...
To learn how the A2A protocol enables this integration, see xref:connect:a2a-concepts.adoc[].
...
Store the client ID and secret in secure credential stores, not in code. Use environment variables, or store the secret in the Agentic Data Plane secret store and reference it by name. Rotate credentials if compromised and restrict access based on the principle of least privilege.
