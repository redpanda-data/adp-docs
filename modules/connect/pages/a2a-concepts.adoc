...
.A2A separates discovery from authenticated message and task exchange
image::a2a-message-lifecycle.svg[alt="A2A lifecycle. An application, agent, or Redpanda Connect pipeline fetches an Agentic Data Plane agent card, chooses a supported interface and protocol version, obtains an access token, sends a message, and receives a direct response or streamed task status and artifacts."]

For the complete specification, see link:https://a2a-protocol.org/latest/specification/[A2A protocol specification^].

== Agent cards

Every A2A-compliant agent exposes an agent card at a well-known URL.

The agent card is a JSON document that describes what the agent can do and how to interact with it. For the complete agent card specification, see link:https://a2a-protocol.org/latest/specification/#44-agent-card[A2A Agent Card specification^].
...
Callers authenticate as themselves, never as the agent they're calling. Each caller needs its own service account and permission to invoke the agent. See xref:connect:concepts.adoc#service-account-authorization[Service account authorization] and xref:control:permissions-reference.adoc#a2a-runtime-permissions[A2A runtime permissions].
...
External applications and agents authenticate with their own service account credentials, not the target agent's. Grant the caller permission to invoke the agent with an access policy or a custom role. See xref:control:access-policies.adoc[].
