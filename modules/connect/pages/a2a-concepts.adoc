...
. Exchange the caller's client ID and secret for a time-limited access token through OAuth2.
. Include the access token in the Authorization header when calling the agent endpoint.
. When the token expires, exchange credentials again for a new token.
