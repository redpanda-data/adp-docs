...
* It appears in the agent registry (`AgentRegistryService.ListAgents`), which means it shows up alongside managed agents and in cost-attribution queries.
...
Once registered, your agent appears in the agent registry (`AgentRegistryService.ListAgents`). The first time it serves a request and emits telemetry, transcripts begin populating.
...
. *Registry*: The agent appears in `AgentRegistryService.ListAgents` and in cost-attribution queries.
...
|Agent registered but doesn't appear in `AgentRegistryService.ListAgents`
|Confirm `ListAgents` returns the agent. If `ListAgents` is empty, the registration didn't persist; retry.
