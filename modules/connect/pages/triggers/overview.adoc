...
NOTE: Triggers are available for Redpanda-managed agents only. A self-managed (BYOA) agent runs on your own infrastructure and is invoked through it, so it has no *Triggers* tab. To bring a self-managed agent into ADP for observability and governance, see xref:connect:byoa-register.adoc[Register your own agent (BYOA)].
