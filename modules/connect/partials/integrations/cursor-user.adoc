...
See xref:ai-gateway:aggregation.adoc[] for details on configuring deferred tool loading.
...
+
Aggregating tools from multiple MCP servers adds processing time. Use deferred tool loading to reduce this overhead (see xref:ai-gateway:aggregation.adoc[]).
