...
* Test agents with the xref:agents:system-prompts.adoc#evaluation-and-testing[systematic testing approach]
...
* Follow tool design patterns in xref:mcp:overview.adoc[]
...
For comprehensive guidance on monitoring agent activity, analyzing conversation history, tracking token usage, and debugging issues, see xref:agents:monitor.adoc[].
