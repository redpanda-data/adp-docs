...
* Test agents with the xref:prompt-best-practices.adoc#evaluation-and-testing[systematic testing approach]
...
* Set max iterations appropriate for task complexity (see xref:concepts.adoc#why-iterations-matter[Why iterations matter])
...
* See cost calculation guidance in xref:concepts.adoc#cost-calculation[Cost calculation]
...
* See xref:a2a-concepts.adoc#agent-card-location[Agent card location] for details
...
* See integration patterns in xref:pipeline-integration-patterns.adoc[]

== Monitor and debug agents

For comprehensive guidance on monitoring agent activity, analyzing conversation history, tracking token usage, and debugging issues, see xref:monitor-agents.adoc[].
