...
For foundational concepts on how agents execute and manage complexity, see xref:agents:concepts.adoc[].
...
For implementation details on external A2A integration, see xref:agents:integration-overview.adoc[].
...
Limit tools per agent to 10-15 for optimal performance. Agents with more than 20-25 tools often show degraded tool selection accuracy. Use subagents to partition tools by domain. For tool design patterns, see xref:mcp:overview.adoc[].
