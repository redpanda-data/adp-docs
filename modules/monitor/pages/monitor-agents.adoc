...
:description: Monitor agent execution, inspect available MCP tools, analyze conversation history, track token usage, and debug issues using Inspector and Transcripts.
:page-topic-type: how-to
:personas: agent_builder, platform_engineer, domain_evaluator
:learning-objective-1: pass:q[Verify agent behavior and review its MCP capabilities using the *Inspector* tab]
...
You must have a running agent. If you do not have one, see xref:get-started:agent-builder-quickstart.adoc[].
...
.Agent Inspector closes the loop between a test prompt and production evidence
image::agent-inspector-test-loop.svg[alt="Agent Inspector test loop. A user sends a prompt, the agent streams responses, artifacts, and MCP tool calls, and Inspector shows context and token usage. The user can retry or clear context, while completed runs remain available in transcripts and activity views."]
...
For a managed agent, the Inspector composer includes a *Tools* control. Use it to review the MCP servers and related capabilities available to the agent before you send a test message.

.Inspector shows tool access through direct MCP servers and subagents
image::inspector-mcp-tool-discovery.svg[alt="Inspector tool discovery. The Tools control displays direct MCP servers and subagents. Direct servers expose their tools to the agent. Each subagent exposes only the tools from the MCP servers configured for that subagent. Authentication and availability affect which tools are visible."]

. Open the agent's *Inspector* tab.
. Click *Tools* in the composer.
. Under Direct MCP servers, expand a server to see its type, description, and exposed tools. Each tool entry shows its name and description when the server provides one.
. Under Subagents, review the subagents this agent can delegate to and the MCP servers configured on each subagent.
...
Some remote MCP servers require you to connect your own account before their tools are shown. Expand the server and click *Connect*, complete the authorization in the new browser tab, then return to Inspector. The list refreshes when you return. If it does not, click *Refresh tools*.
