...
:description: Monitor agent execution, inspect available MCP tools, analyze conversation history, track token usage, and debug issues using Playground and Transcripts.
:page-topic-type: how-to
:personas: agent_builder, platform_engineer, domain_evaluator
:learning-objective-1: pass:q[Verify agent behavior and review its MCP capabilities using the *Playground* tab]
...
For a managed agent, the Playground composer includes a *Tools* control. Use it to review the MCP servers and related capabilities available to the agent before you send a test message.
