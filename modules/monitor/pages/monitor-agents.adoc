...
The list reflects the agent's current configuration and the tools each direct MCP server exposes. It does not guarantee that a later tool call will succeed. Server availability, access policy, or an external connection can still affect a call at runtime.
