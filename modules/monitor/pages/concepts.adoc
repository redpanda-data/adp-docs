...
Captured request, response, or configuration data may contain sensitive information. Agentic Data Plane redacts fields marked as sensitive and may truncate or omit captured data. Review an event before sharing it outside your organization.

The Audit Log view returns events for your Agentic Data Plane environment. Access requires an audit log permission, and viewing the audit log is itself recorded as an audit event. See xref:control:permissions-reference.adoc#audit-log-permissions[Audit log permissions].
