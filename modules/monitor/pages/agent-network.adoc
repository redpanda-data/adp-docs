...
// Monitor health section re-verified against apps/adp-ui/src/components/network/{network-view.tsx,network-status.ts,network-metrics.ts,network-detail-panel.tsx} on origin/main, 2026-08-08: the attention strip now surfaces every MCP server in error, not only credential failures. Each row names the cause (an authorization failure, or a percentage of tool calls failing in the window), how many agents are affected, and how many calls failed. Feature-gated preview, enabled per organization.
...
The view flags every MCP server that needs attention, whether its credentials fail to resolve (for example, an expired or missing user connection) or its tool calls are failing on their own. Each alert names the cause, how many agents are affected, and how many calls failed, so you can act before more calls fail. For a credential failure, reconnect the user connection; see xref:connect:user-delegated-oauth.adoc[] for managing user connections.
