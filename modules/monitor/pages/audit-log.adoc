...
A deployment can therefore show the page with no recorded events. If the table is empty for a period you know had activity, see <<troubleshoot-the-audit-log,Troubleshooting>>.
====

== Prerequisites

Your account needs the `dataplane_adp_auditlog_list` permission to open the audit log and view event details. Outside the Admin role, audit log reads come from an xref:control:access-policies.adoc[access policy] naming `Action::"AuditLog.list"`. See xref:control:permissions-reference.adoc#audit-log-permissions[Audit log permissions].
...
// The Subsystem filter values below mirror the UI labels exactly, and the UI label is lowercase-g "MCP gateway"
// (apps/adp-ui/src/routes/_authenticated/audit-log.utils.ts, verified against origin/main 2026-08-06).
// Keep these two mentions lowercase even though the product name is written "MCP Gateway" elsewhere.
...
|Ask an administrator to confirm that your account has `dataplane_adp_auditlog_list`, from the Admin role or from an xref:control:access-policies.adoc[access policy] naming `Action::"AuditLog.list"`.
...
* xref:monitor:concepts.adoc#how-redpanda-stores-audit-events[How Redpanda stores audit events]
