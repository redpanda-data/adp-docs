...
. Verify MCP server status in *Remote MCP*.
...
* Test agents with the xref:connect:system-prompts.adoc#evaluation-and-testing[systematic testing approach]
...
* Follow tool design patterns in xref:connect:mcp-overview.adoc[]
...
* Set max iterations appropriate for task complexity (see xref:connect:concepts.adoc#why-iterations-matter[Why iterations matter])
...
. Clear conversation history periodically in the *Playground* tab.
...
* See cost calculation guidance in xref:connect:concepts.adoc#cost-calculation[Cost calculation]
...
processors:
  - a2a_message:
      agent_card_url: "https://your-agent-id.ai-agents.your-cluster-id.clusters.rdpa.co"
...
processors:
  - a2a_message:
      agent_card_url: "https://your-agent-id.ai-agents.your-cluster-id.clusters.rdpa.co/.well-known/agent-card.json"
      prompt: "Analyze this transaction: ${!content()}"
----
...
* See xref:connect:a2a-concepts.adoc#agent-card-location[Agent card location] for details
...
* See integration patterns in xref:connect:pipeline-integration-patterns.adoc[]

== Monitor and debug agents

For comprehensive guidance on monitoring agent activity, analyzing conversation history, tracking token usage, and debugging issues, see xref:monitor:monitor-agents.adoc[].
