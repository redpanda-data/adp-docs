...
The transcript shows token usage per turn. For spend analysis across agents, models, and users, open *Cost & Usage* under *Governance* in the sidebar (see xref:control:cost-usage.adoc[Analyze Cost and Usage]), or use breakdown queries through `SpendingService`; per-transcript `estimated_cost_usd` is also available through the Transcripts API.
