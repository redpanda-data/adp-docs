...
* You have access to Agentic Data Plane.
...
Use logs for: Debugging specific requests, viewing full prompts/responses. See xref:monitor:logs.adoc[].

== Where to find metrics

1. Navigate to analytics dashboard:
   * In the sidebar, navigate to *AI Gateway* > *Gateways* > *{gateway-name}*, then select the *Analytics* tab.
...
* By custom header (if configured, for example, `x-customer-id`)
...
* Set SLO targets (for example, "p95 < 2 seconds")
...
* Total error rate (%)
* Errors by status code (400, 401, 429, 500, and so on)
* Errors by model
* Errors by provider
...
* Identify configuration issues (for example, model not enabled)
...
* Spend by custom dimension (if configured, for example, customer ID)
...
* *OpenTelemetry*: Traces exported to Redpanda topics through the OpenTelemetry exporter
...
2. Identify spike (for example, Jan 10th: $500 vs usual $100)
3. Drill down:
...
* Seasonal patterns (for example, more usage on weekdays)
...
2. Send test request (see xref:get-started:gateway-quickstart.adoc[])
3. Check permissions with admin

== Next steps

* xref:monitor:logs.adoc[]
