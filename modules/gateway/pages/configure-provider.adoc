...
// TODO: confirm against the latest UI (demo recording/screenshots): the detail-page KPI strip and where its cards link now that Cost & Usage lives under Governance.
The detail page also carries a Last 7 days KPI strip (`TOTAL SPEND`, `REQUESTS`, `TOKENS`) with sparklines and _vs previous period_ deltas, and a usage chart scoped to this provider. For analysis across providers, use the *Cost & Usage* page under *Governance* (see <<view-cost-and-usage>>).
