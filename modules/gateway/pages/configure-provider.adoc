...
// Create-form labels re-verified against apps/adp-ui/src on cloudv2 origin/main, 2026-08-06: the Identity section shows only Display name (the resource ID is derived silently and never rendered on the create form); the Credential section is a choicebox of existing keys plus "Bring a new API key reference" (Key name + API key + Save key); the Existing/New tabs, Secret value field, and Create secret button exist only on the edit page's API key reference field; the Models picker starts with the full catalog selected; deleting requires typing "delete" to confirm.
...
After you create the provider, its detail page organizes everything about it into tabs: *Overview*, *Models*, *Connect*, and *Settings*.
...
The *Models* tab lists the chat models available on this provider. Each row shows the model's name and identifier, icons for the capabilities it supports (such as vision, tools, JSON, or reasoning), its context-window limit, and its input and output prices per million tokens. Hover over a capability icon to read what that capability means. Use the toggle on a row to enable or disable that model on the provider. Select a model to open its detail page (see <<model-detail-page>>).
...
* Pricing: A read-only rate card of the model's effective per-bucket rates, that is, the catalog rates with any per-model overrides applied (see <<pricing-overrides>>). This section appears only when rates are known, and links to the upstream provider's pricing reference.
...
. Click *Create provider*. Until the form is complete, the button stays disabled and the hint next to it tells you what's still needed (for example, saving the new key).
. On the provider's detail page, the Connection card shows the provider type, `Status`, `Proxy URL`, `Base URL`, and `API key reference`. Copy the `Proxy URL`: this is where your applications point.
. Click *Test connection*. The result appears as a pass or fail message.
+
// TODO: re-verify this save-and-verify flow end to end. The Test connection button now lives on the provider edit page with no model picker, no "Verify connection" section heading, and no "Show commands" disclosure; confirm where the docs should send readers after create.
...
Use the search box to find a provider by name, and the *Filter* button to narrow the list by provider type, model, or status. The *Add provider* button opens the create flow described in <<open-the-create-llm-provider-page,Open the Create LLM provider page>>. Each row's actions menu can activate or deactivate the provider, copy its proxy or base URL, or delete it. The list paginates, with a rows-per-page selector in the footer.

[[view-cost-and-usage]]
== View cost and usage

The *Cost & Usage* page under *Governance* tracks spend, request volume, and token volume over time across providers, models, agents, and users, with period comparisons and CSV export. This report moved to its own page: see xref:control:cost-usage.adoc[Analyze Cost and Usage].
...
* *Delete*: Scroll to the Delete this provider section at the bottom of the detail page, click *Delete*, and type `delete` to confirm. The action is permanent. In-flight requests fail and downstream clients receive errors until reconfigured.
...
|Confirm your account can act on LLM providers. Listing them needs `Action::"LLMProvider.list"` and creating one needs `Action::"LLMProvider.create"`. Outside the Admin role, both come from an xref:control:access-policies.adoc[access policy]. See xref:control:permissions-reference.adoc#llm-provider-permissions[LLM provider permissions].
|===
