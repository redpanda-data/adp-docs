...
The *Models* tab lists the chat models this provider exposes. Each row shows the model's name and identifier, capability chips (such as `vision`, `tools`, `json`, or `reasoning`), its context-window limit, and its input and output prices per million tokens. Use the toggle on a row to enable or disable that model on the provider. Select a model to open its detail page (see <<model-detail-page>>).
...
|`Provider`
|The provider-type icon (OpenAI, Anthropic, Google, AWS Bedrock, or OpenAI-compatible), the display name, and the resource identifier beneath it. Select the row to open the provider's detail page.
