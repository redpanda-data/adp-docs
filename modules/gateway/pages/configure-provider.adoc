...
AI Gateway preserves the regional prefix end to end when it records spend, so the xref:control:dashboard/overview.adoc[governance dashboard] and the *Cost & Usage* tab attribute usage to the correct regional rate. A call to `eu.anthropic.claude-haiku-4-5` is billed at the EU Haiku rate, not the headline foundation-model rate.
