...
When the *Playground* tab is available, use it to test one of the provider's models before you connect application code. Choose a model, send a prompt, and optionally adjust response controls such as reasoning effort and maximum output tokens. The response streams into the conversation while the Inspector shows the request, response events, status, and timing. Playground conversations are ephemeral.

.Playground turns a model test into an inspectable gateway request
image::llm-playground-test-loop.svg[alt="LLM Playground test loop. A user selects a model, writes a prompt, and chooses response controls. AI Gateway translates and sends the request to the configured provider, streams the response back, and exposes request details, response events, status, and timing in the Inspector."]
...
+
// TODO: re-verify this save-and-verify flow end to end. The Test connection button now lives on the provider edit page with no model picker, no "Verify connection" section heading, and no "Show commands" disclosure; confirm where the docs should send readers after create, and whether the Connection card fields listed above still match.
