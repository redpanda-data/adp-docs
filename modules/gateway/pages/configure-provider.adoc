...
// Legacy gateway-object-model pages (gateway-quickstart.adoc, gateway-architecture.adoc, admin/setup-guide.adoc, builders/discover-gateways.adoc) were archived to gateway/partials/archived-ai-gateway-pages.adoc and removed from the nav. aggregation.adoc (now "MCP Aggregation") is a current page and was kept. Old cloud URLs redirect via :page-aliases: on gateway/overview.adoc.
...
[IMPORTANT]
====
Gemini uses the `x-goog-api-key` header for authentication, not `Authorization: Bearer`. This matters when you wire up clients. See xref:gateway:connect-agent.adoc[Connect your app to an LLM provider].
====
...
. To wire up an application, open *Connect your app* further down the page or follow xref:gateway:connect-agent.adoc[Connect your app to an LLM provider].
...
|Gemini returns 401
|Gemini uses the `x-goog-api-key` header, not `Authorization`. If you're seeing 401s on Gemini, check that the client is sending the correct header. See xref:gateway:connect-agent.adoc[Connect your app to an LLM provider].
...
* xref:gateway:connect-agent.adoc[Connect your app to an LLM provider]
