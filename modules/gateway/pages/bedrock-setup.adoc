...
:description: Create the IAM user, policy, and access keys required for AI Gateway to invoke Amazon Bedrock models, then register the provider in Agentic Data Plane.
...
. Open *LLM Providers* in the sidebar and click *Add provider*.
...
.. Paste the `AccessKeyId` value from the IAM user setup and click *Create*. The secret is stored in the Agentic Data Plane secret store, scoped to AI Gateway.
...
. Verify the provider by clicking *Test connection* on its edit page. A successful response confirms that the credentials, region, and IAM policy are correctly configured.
