...
TIP: The provider's *Connect* tab in Agentic Data Plane generates this configuration for you: a gateway-token step, setup instructions for popular clients, and code examples with the provider's proxy URL prefilled. Copy from the tab for a quick start, or follow this page for the full flow.
...
* For CI or programmatic clients: Your agent's service account client ID and a client secret, issued from the agent's *Credentials* tab. See xref:connect:concepts.adoc#service-account-authorization[Service account authorization]. To register an agent and issue credentials, see xref:connect:self-managed-agents.adoc[].
...
. Select the Agentic Data Plane environment whose AI Gateway you want to target. The `rpk ai env use` command accepts an environment name or ID and switches the active environment:
+
[source,bash]
----
rpk ai env list
...
|403
|Forbidden. Nothing grants the service account this action, or the provider is disabled.
...
* The service account may lack the required access. Ask an admin for an xref:control:access-policies.adoc[access policy] that names the service account as its principal and grants `Action::"LLMProvider.invoke"`, plus `Action::"LLMProvider.get"` if the account also reads provider config. No built-in role short of Admin grants these. See xref:control:permissions-reference.adoc#llm-provider-permissions[LLM provider permissions].
