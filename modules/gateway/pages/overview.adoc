...
* *My Connections*: Your own per-user OAuth connections to the configured upstream providers, so MCP servers can act on your behalf.
* *Integrations setup*: Register the OAuth plumbing behind MCP tools. The *Outbound providers* tab holds upstream OAuth providers for user-delegated MCP authentication (for example, GitHub or Google); the *Inbound clients* tab holds external tools (Claude.ai, ChatGPT, Cursor) that request access tokens from the gateway.
