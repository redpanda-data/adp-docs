= MCP Aggregation
:description: Learn how to aggregate MCP servers, configure deferred tool loading, create orchestrator workflows, and manage security.
...
glossterm:MCP[,Model Context Protocol (MCP)] aggregation lets glossterm:AI agent[,AI agents] access glossterm:MCP tool[,tools] from multiple MCP servers through a single unified endpoint. This eliminates the need for agents to manage multiple MCP connections and significantly reduces token costs through deferred tool loading.
...
flowchart TB
    ...
    Agent -->|"1. Discover tools via /mcp endpoint<br/>2. Invoke specific tool"| Gateway

    subgraph Gateway["AI Gateway (MCP Aggregator)"]
        direction TB
        Deferred["Deferred tool loading<br/>(search + orchestrator sent initially, others deferred)"]
        Orchestrator["Orchestrator in JavaScript<br/>(reduces round trips for multi-step workflows)"]
        Registry["Approved MCP server registry<br/>(admin-controlled)"]
    end

    Gateway -->|"Routes to appropriate MCP server"| DB["MCP Database server"]
    Gateway --> FS["MCP Filesystem server"]
    Gateway --> Slack["MCP Slack server"]
    Gateway --> Search["MCP Search server"]
....
...
1. Navigate to MCP servers:
   ...
   * In the sidebar, navigate to *AI Gateway* > *Gateways*, select your gateway, then select the *MCP* tab.
...
1. Verify MCP server is active in Redpanda ADP
...
* Standard gateway authentication (ADP token)
