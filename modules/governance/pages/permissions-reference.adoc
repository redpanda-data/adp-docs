...
For an explanation of how permissions, roles, and role bindings fit together, see xref:governance:permissions-overview.adoc[About ADP Permissions].
...
The `dataplane_adp_spending_*` permissions gate the governance APIs that surface AI spend, request counts, and token volume. See xref:governance:budgets.adoc[Token Budgets and Limits] for what spending data ADP records automatically.
...
* xref:redpanda-cloud:security:authorization/rbac/rbac.adoc[Role-Based Access Control] in Redpanda Cloud
