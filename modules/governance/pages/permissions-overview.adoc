...
ADP permissions live in six families, named by the namespace prefix:
...
ADP includes seven built-in roles. The first three are general-purpose; the last four are narrow Invoker roles for runtime-only access.
...
For the Cloud-wide RBAC mechanics (principals, scopes, custom roles, role-binding API), see xref:redpanda-cloud:security:authorization/rbac/rbac.adoc[Role-Based Access Control].
