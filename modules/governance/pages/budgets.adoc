...
|Time-series spend, request, and token charts across providers and models. Use it to group by provider, model, or token type, then filter by provider, model, cost type, token type, or user. See xref:ai-gateway:configure-provider.adoc#view-cost-and-usage[View cost and usage].
