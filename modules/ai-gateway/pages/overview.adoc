...
The provider's detail page in the Cloud UI records request and token counts. A *Cost & usage* section is in development (the UI shows a "Coming soon" placeholder today).

== What's in the UI

// TODO: this guide describes the cluster-embedded view available today on cloud.redpanda.com. The standalone-ADP UI launches as a separate product surface; sign-in URL, IAM model, and sidebar layout will change. Update this section once standalone ADP ships.

In Redpanda Cloud, open your cluster and expand the *ADP* section in the sidebar. You'll see four sub-items:
