...
This page provides technical details about AI Gateway's architecture, request processing, and capabilities. For an overview of AI Gateway, see xref:ai-gateway:overview.adoc[]
...
Built-in fallback behavior when primary providers are unavailable (configurable through preference toggles).

*6 User Preference Toggles:*

Configurable preferences that influence routing without modifying rules (see xref:admin/configure-ai-hub.adoc[] for details).
...
See xref:admin/eject-to-custom-mode.adoc[] for the ejection process.
endif::[]
...
* xref:ai-gateway:gateway-quickstart.adoc[]
