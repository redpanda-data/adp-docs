...
|You called a Claude 4.6 or later model with a bare foundation-model ID. Switch to an inference profile, for example `us.anthropic.claude-sonnet-4-6` instead of `anthropic.claude-sonnet-4-6`. See xref:ai-gateway:configure-provider.adoc#bedrock-inference-profiles[AWS Bedrock: Inference profiles and IAM].
|===
