...
For detailed comparison, see xref:gateway-modes.adoc[].
...
+
For advanced routing based on request properties, use CEL expressions. See xref:routing-cel.adoc[] for examples.
+
...
See xref:aggregation.adoc[] for detailed information about MCP aggregation.
