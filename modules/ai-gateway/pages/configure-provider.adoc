...
[IMPORTANT]
====
Gemini uses the `x-goog-api-key` header for authentication, not `Authorization: Bearer`. This matters when you wire up clients. See xref:ai-gateway:connect-agent.adoc[Connect your agent].
====
...
. To wire up an application, open *Connect your app* further down the page or follow xref:ai-gateway:connect-agent.adoc[Connect your agent].
