...
|**Homepage URL**
|Your Redpanda ADP cluster URL, for example `\https://aigw.<cluster-id>.clusters.rdpa.co`
...
*This must match exactly.* Replace `<cluster-id>` with your actual cluster ID.
|===
+
. Click **Register application**.
. On the application page, note your **Client ID** (for example, `Iv1.abc123...`).
...
== Store the GitHub client secret in ADP

Before creating the OAuth provider in ADP, store the client secret in ADP's secret store.

. Sign in to ADP
. Navigate to **Secrets Store** in the sidebar
. Click **Create secret**
. Configure the secret:
+
* **Secret ID**: `GITHUB_CLIENT_SECRET` (must be UPPER_SNAKE_CASE)
* **Secret value**: Paste the client secret from GitHub
* **Scopes**: Select `SCOPE_REDPANDA_CONNECT` and `SCOPE_REDPANDA_CLUSTER` (`AI GATEWAY` or `MCP Server` or `AI AGENT`) ???
+
. Click **Create secret**

The secret reference `GITHUB_CLIENT_SECRET` is what you'll use when creating the OAuth provider.
...
. Fill in the provider details using the following field-by-field guide:
...
|**Client ID** *
|`Iv1.abc123...`
|The Client ID from your GitHub OAuth app. Found in GitHub Settings → Developer settings → OAuth Apps → [Your App].
...
|The UPPER_SNAKE_CASE reference to the secret you created earlier. Must match exactly.
|===

=== Scopes

Define the GitHub scopes your MCP servers will need. Click in the **Scopes** field and add these common scopes:
...
. In ADP, navigate to **My Connections** in the sidebar (under ADP → My Connections)
...
* Check that the **Authorization callback URL** in your GitHub OAuth app exactly matches `\https://aigw.<cluster-id>.clusters.rdpa.co/oauth/v1/callback`
...
* Review the scopes - ensure they're valid GitHub OAuth scopes

See the <<Troubleshooting>> section at the end of this tutorial for more debugging tips.
====
...
. In ADP, navigate to **MCP Servers** in the sidebar (under ADP → MCP Servers).
. Click **Create server**.
...
|**Code mode**
|Toggle on/off
|Enable to add search and execute tools that let agents run sandboxed code against this server's tools. Typically enabled for development/testing.
...
|Minimum OAuth scopes a user's connection must have. If a user's stored connection has fewer scopes than specified here, they'll be prompted to re-consent with upgraded scopes. For read-only GitHub access, the managed MCP typically requires scopes like `read:user`, `read:org`, and `repo`.
|===
...
Each user who calls tools from this MCP server will authenticate with their own GitHub account. User tokens are stored securely in ADP's token vault and automatically refreshed before expiry.
