...
* For *managed* MCP servers: the type must support user-delegated OAuth. SQL doesn't; Slack, Jira, and Google managed types do. Check xref:managed-catalog.adoc[] before configuring.

== Configure the server

. Create or edit your MCP server (see xref:create-server.adoc[]).
...
* xref:managed/slack.adoc[] — consumer-facing user-delegated OAuth example. Shows the consent flow against a real Slack workspace.
* xref:managed/jira.adoc[] — enterprise user-delegated OAuth example. Atlassian's OAuth flow differs from Slack's; this page calls out scope-management gotchas.
