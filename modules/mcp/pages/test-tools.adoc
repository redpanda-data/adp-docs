...
The Inspector is a built-in MCP client embedded in the ADP UI. It runs on top of the same JSON-RPC connection that agents use, so a tool that works in the Inspector will work for an agent. Use the Inspector to verify your server end-to-end after creating it (xref:create-server.adoc[]) or whenever you change a tool's schema.
...
|User-delegated auth has no stored token for the calling user. Redpanda includes an `authorize_url` in the error detail; complete the consent flow per xref:user-delegated-oauth.adoc[].
...
* *Aggregating multiple servers* — see xref:ai-gateway:aggregation.adoc[].
