...
.`rpk ai` reuses one authenticated environment across resource, protocol, and agent workflows
image::rpk-ai-workflow.svg[alt="rpk ai workflow. Install the rpk ai plugin, sign in and select an Agentic Data Plane environment, then manage LLM providers, MCP servers, OAuth resources, and agents; list or call MCP tools and send A2A messages; or launch supported coding agents through AI Gateway. Each command shares the selected identity, environment, and resource scope."]
