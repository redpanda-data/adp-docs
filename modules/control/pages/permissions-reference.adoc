...
These permissions control who may author policies. They are not the actions a policy can name: a policy names an entity type and a verb, such as `Action::"Policy.create"`. See xref:control:access-policies.adoc#action-reference[Action reference].
...
The `dataplane_adp_effectivepolicyset_*` permissions gate read access to the derived policy set the dataplane enforces: your access policies plus the permits derived from role bindings. It's an audit view for highly privileged users, not something you author.
