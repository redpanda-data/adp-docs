...
:description: Look up the permissions Agentic Data Plane enforces across LLM providers, MCP servers, agents, pipelines, knowledge bases, and governance APIs, and the built-in roles that grant them.
...
For an explanation of how permissions, roles, and role bindings fit together, see xref:control:permissions-overview.adoc[Control Who Can Do What].
...
The Admin role grants every permission and is omitted from individual rows for brevity. Permissions that only resolve on Agentic Data Plane-enabled clusters carry no separate mark; the gating is automatic.
...
The `dataplane_adp_policy_*` permissions gate the Cedar-based access policies that refine RBAC. Access policies are a preview capability. See xref:control:access-policies.adoc[].

These permissions control who may author policies. They are not the actions a policy can name, and the two naming schemes don't translate mechanically into each other. Look up policy actions in xref:control:access-policies.adoc#action-reference[Action reference].
...
|List the permits derived from role bindings
|✓
|✓
|===

`dataplane_adp_systempolicy_list` is separate from `dataplane_adp_policy_list` because the derived view names the principal of every role binding, which is a wider disclosure than listing the access policies a caller already owns. Grant it on its own to let someone review the organization's RBAC projection without granting access-policy reads.

== Policy template permissions

The `dataplane_adp_policytemplate_*` permissions gate reusable access-policy templates, including the built-in quick-start set. See xref:control:access-policies.adoc[].
...
== Effective policy set permissions

The `dataplane_adp_effectivepolicyset_*` permissions gate read access to the derived policy set the dataplane enforces: your access policies plus the permits derived from role bindings. Only highly privileged users can read it, and it's read-only.
...
|View the derived effective policy set
|✓
|✓

|`dataplane_adp_effectivepolicyset_list`
|List the derived effective policy sets
|✓
|✓
|===
...
The `dataplane_pipeline_*` permissions gate Redpanda Connect pipelines used by Agentic Data Plane for ingestion and transformation. The PipelineInvoker role grants only the runtime invocation permissions.
...
|Writer
|Developers who build and modify Agentic Data Plane resources. Grants full CRUD on every Agentic Data Plane-namespaced API plus pipeline and knowledge-base management.

|Reader
|Auditors and evaluators who need visibility without mutation rights. Grants `_get` and `_list` plus MCP runtime read methods.

|TranscriptReader
|Users and service accounts that read agent conversation transcripts. Grants `dataplane_adp_transcript_get` and `dataplane_adp_transcript_list`, which stay out of the Writer and Reader roles because transcripts carry full conversation content. Provisioned for organizations with an Agentic Data Plane cluster.
