...
[IMPORTANT]
====
These permission strings are the RBAC-side vocabulary. They are not what you write to grant access.
...
The two Redpanda Connect namespaces at the end of this page behave differently. <<pipeline-permissions,Pipeline>> and <<knowledge-base-permissions,knowledge base>> permissions are owned outside Agentic Data Plane, so they remain part of Writer and Reader and keep their role columns.
...
`dataplane_adp_llmprovider_invoke` is the one permission an application needs to send traffic through AI Gateway. It is the narrowest useful grant for a service account, and no built-in role provides it on its own. Grant it with an access policy naming `Action::"LLMProvider.invoke"`.
...
[NOTE]
====
`dataplane_adp_agent_session_get` returns a session's full conversation content (prompts, tool inputs and outputs, and model output), the same class of content that transcript access exposes. Treat it as sensitively as the transcript permissions when a policy grants it.
====
...
`dataplane_adp_systempolicy_list` is separate from `dataplane_adp_policy_list` because the derived view names the principal of every role binding in the organization.
...
`dataplane_adp_a2a_invoke` covers the AI Gateway A2A proxy path, and a policy can grant it by naming `Action::"Agent.invoke"`.
