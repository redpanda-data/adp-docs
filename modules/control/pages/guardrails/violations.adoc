...
When a guardrail acts on a request, the caller sees a blocked message and ADP records the action on the request's trace. This release does not include a dedicated violations dashboard, so you review blocked requests through glossterm:transcript[,transcripts].
...
ADP records guardrail activity as attributes on the request's OpenTelemetry trace, which you read in the request's transcript alongside the LLM call, tool calls, and cost data. See xref:monitor:transcripts.adoc[See what your agent did].
