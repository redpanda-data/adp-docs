...
== Step 3: Turn on policies

All six policies are optional. Turn on only the ones you need, then configure each. For the full configuration of every policy, see xref:control:guardrails/types-reference.adoc[Guardrail policy reference].
...
Send a request through an attached provider that should trip a policy. For example, with the sensitive-information policy set to block on input, send a prompt that contains an email address or other PII.
