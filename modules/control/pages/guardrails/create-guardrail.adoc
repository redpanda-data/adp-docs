...
A guardrail takes effect only after a provider references it. On an LLM provider, set its guardrail to the one you created. A provider references one guardrail, and you can reuse the same guardrail across many providers. See xref:gateway:configure-provider.adoc[Configure an LLM provider].
