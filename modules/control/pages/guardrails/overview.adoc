= How Guardrails Work
:description: Learn how Agentic Data Plane guardrails use AWS Bedrock policies to inspect and control LLM requests and responses, and how a blocked request surfaces.
:page-preview: true
...
.A guardrail can stop a request before the model or control the response after it
image::guardrail-evaluation-flow.svg[alt="Guardrail evaluation flow. A caller's prompt passes through input policies before the Bedrock model. Allowed prompts reach the model; blocked prompts return the configured blocked-input message. For non-streaming calls, model output passes through output policies before an allowed, redacted, or blocked response reaches the caller."]
...
Agentic Data Plane records guardrail activity as attributes on the request's OpenTelemetry trace, which you read in glossterm:transcript[,transcripts]. This release does not include a separate violations dashboard. See xref:control:guardrails/violations.adoc[Review blocked requests] for how a blocked request surfaces.
