...
ADP guardrails run on AWS Bedrock Guardrails. Each guardrail syncs its policy configuration to the Bedrock control plane, so it needs an AWS region and Bedrock credentials, even when it protects a non-Bedrock provider such as OpenAI, Anthropic, or Google. For those providers, ADP runs the guardrail by calling Bedrock's evaluation API around each request.
