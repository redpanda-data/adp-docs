...
A guardrail bundles a set of policies, each backed by AWS Bedrock Guardrails. Each policy is individually optional, but a guardrail must enforce at least one. This page documents each policy's configuration, the actions it can take, and which direction (input, output, or both) it evaluates.
