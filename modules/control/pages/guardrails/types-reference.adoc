...
:description: Reference for the six guardrail policy types, their configuration fields, actions, directions, and limits.
:page-topic-type: reference
:personas: security_compliance_lead, platform_engineer

A guardrail bundles up to six policies, each backed by AWS Bedrock Guardrails. Every policy is optional. This page documents each policy's configuration, the actions it can take, and which direction (input, output, or both) it evaluates.
...
Feature availability varies by AWS region. Choose a region that supports the policies you need, and see the https://docs.aws.amazon.com/bedrock/latest/userguide/guardrails.html[AWS Bedrock Guardrails documentation^] for exhaustive behavior and regional support.
...
|Definition
|What the topic covers, up to 1000 characters. The definition drives the semantic match, so be specific.

|Examples
|Up to five example phrases, each up to 100 characters, that should match the topic.
...
|Action
|Per direction (input and output): `None` (detect), `Block`, or `Anonymize`. `Anonymize` replaces each match in place with its entity type, such as `{EMAIL}`.
|===
...
For RAG-style applications, check model output against a source and the user's query. This policy evaluates output only and has two independent sub-filters.
...
|Policy ARNs
|One or two Bedrock Automated Reasoning policy ARNs to attach. Each ARN must point at a specific numeric version (for example, ending in `:1` or `:2`); the `DRAFT` version is rejected. Create and publish the policies in the AWS Bedrock console first, then reference their versioned ARNs here.
