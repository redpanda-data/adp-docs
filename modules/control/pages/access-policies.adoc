...
Five entity types in this table each cover exactly one instance for the whole deployment: `AgentNetwork`, `AuditLog`, `DCRSettings`, `EffectivePolicySet`, and `Spending`. Pin the type with `resource is <Entity>`, because the single instance carries a fixed internal ID and a narrower resource clause adds nothing. A role binding scoped to an instance of one of these types resolves to nothing, so grant these actions with a policy instead.
