...
Tags are the attribute half of attribute-based access control (ABAC), and they're carried by the resources with a create and update lifecycle. The read-only and derived types don't have them, for the same reason they record no creator: sessions, transcripts, models, agent credentials, OAuth connections, and the derived policy views.
...
* An account that can author access policies. The built-in Admin and Writer roles can create, edit, and delete them; Reader can view them but not change them. See xref:control:permissions-reference.adoc[].
...
The *Roles* and *System policies* tabs are read-only views of what role-based access control (RBAC) already grants.
