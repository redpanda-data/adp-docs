...
Use roles to get started and to stay coarse: provision the first user, and hand out broad administrative roles where per-resource precision isn't the point. Redpanda turns those role bindings into permits for you, so they take part in the same evaluation and you never author them by hand.
...
* An account that can author access policies. The built-in Admin and Writer roles can create, edit, and delete them; Reader can view them but not change them.
...
|`unrecognized action`
|Use an `<Entity>.<verb>` action ID from <<action-reference,Action reference>>. A role permission name is not a policy action, so copy the action ID rather than reusing a permission you found elsewhere.
