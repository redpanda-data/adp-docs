...
* *Resource*: What the action targets, as a typed entity, for example `resource is Agent` for every agent or `resource == Agent::"support-bot"` for one. Entity IDs are the resource's own name, the same value the API and `rpk ai` use.
...
Three rules govern how a request resolves:
...
** *Template*: link to a reusable template. The principal you set still applies, but the template's own action set and its own effect take over, so the *Effect* and *Conditions* you set here are ignored.
