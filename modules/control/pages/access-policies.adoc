...
`SystemPolicy.list` gates reading the permits derived from role bindings. That view names every binding's principal, so it sits on its own action rather than riding along with `Policy.list`.
...
|Tags
|`Agent`, `Budget`, `Guardrail`, `LLMProvider`, `OAuthClient`, `OAuthProvider`, `Policy`
...
|`changed_tags`
|`Agent`, `Budget`, `Guardrail`, `LLMProvider`, `OAuthClient`, `OAuthProvider`, on update only
|`resource has changed_tags && resource.changed_tags.contains("<key>")`
|===

Tags are the attribute-based half of ABAC, and they're carried by the resources with a create and update lifecycle. The read-only and derived types don't have them, for the same reason they record no creator: sessions, transcripts, models, agent credentials, OAuth connections, and the derived policy views.

`Policy` carries tags but no `changed_tags`, so you can condition on a policy's current tags but not on a tag transition.

`created_by` and `updated_by` hold `User` references, so compare them to `principal` directly rather than to a string.
...
NOTE: `McpServer` carries no tags yet. Tag-based conditions on MCP servers arrive with the next Agentic Data Plane API version. Scope MCP policies by resource instead, with `resource == McpServer::"<name>"`.
...
`changed_tags` exists only on the update path, so this policy doesn't fire on create. To close the create case too, pair the policy with a create-scoped clause that reads the tag directly:
