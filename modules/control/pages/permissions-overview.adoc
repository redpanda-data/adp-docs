...
== Access policies: refine RBAC with conditions

As a preview capability, you can layer xref:control:access-policies.adoc[access policies] on top of RBAC. Access policies are Cedar-based `permit` and `forbid` statements that express what roles can't. They condition on resource tags and ownership, scope down to a single resource, and deny across every role that would otherwise grant access.

The two layers compose with a simple rule: a request is allowed when a `permit` matches it and no `forbid` matches it. Your role bindings participate as automatically generated permits, so RBAC keeps working unchanged, and a `forbid` always wins, including over access that a role grants.
