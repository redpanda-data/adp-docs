...
:description: Understand how Agentic Data Plane permissions, built-in roles, and role bindings fit together to control access to AI Gateway, MCP, agents, pipelines, and governance APIs.
:page-topic-type: concept
:personas: security_compliance_lead, platform_engineer
:learning-objective-1: Explain how Agentic Data Plane permissions, roles, and role bindings fit together
...
Redpanda Agentic Data Plane uses role-based access control (RBAC) to gate every API call, so you can apply least-privilege access across your deployment and keep developers, operators, and service accounts to exactly what they need. RBAC in Agentic Data Plane composes permissions, roles, and role bindings, with scopes that range from the entire organization down to a single dataplane.

Agentic Data Plane ships built-in roles for common patterns (Admin, Writer, Reader, and task-specific Invoker roles) and supports custom roles for more granular control. This page explains the three-layer model, the Agentic Data Plane permission families, and how to pick the right role for each workload.
...
Redpanda Agentic Data Plane authorization composes multiple layers. Each layer answers a different question.
...
* *Role*: Which permissions go together? A role is a named bundle of permissions. Agentic Data Plane ships built-in roles for common access patterns (Admin, Writer, Reader, and a set of Invoker sub-roles), and you can define custom roles for more specific needs.
...
== Agentic Data Plane-namespaced permissions

Agentic Data Plane permissions live in the following families, named by the namespace prefix:
...
Two more families belong to Redpanda Connect resources that Agentic Data Plane uses:
...
Agentic Data Plane includes the following built-in roles. Admin, Writer, and Reader are general-purpose. The remaining roles are narrow: a set of Invoker roles for runtime-only access, plus TranscriptReader for reading agent conversation transcripts.
...
|Writer
|Full create, read, update, and delete access across every Agentic Data Plane-namespaced API (LLM provider, MCP server, agent, agent credential, AI Gateway, pipeline, knowledge base). Use for developers who need to build and modify Agentic Data Plane resources.
...
|TranscriptReader
|Transcript reads only: `dataplane_adp_transcript_get` and `dataplane_adp_transcript_list`. Transcripts carry full conversation content, so this access is granted through a dedicated role rather than bundled into Reader or Writer. Use for users and service accounts that read agent conversation transcripts. Provisioned for organizations with an Agentic Data Plane cluster.
...
== Where access policies take over

As a preview capability, xref:control:access-policies.adoc[access policies] carry the authorization decision. Access policies are Cedar-based `permit` and `forbid` statements that express what roles can't. They condition on resource tags and ownership, scope down to a single resource, and deny across every role that would otherwise grant access.

With access policies enabled, use RBAC for what it's good at: provisioning the first user, and handing out broad administrative roles where per-resource precision isn't the point. Redpanda turns those role bindings into permits, so they take part in the same evaluation. Everything finer than that belongs in a policy.

A request is allowed when a `permit` matches it and no `forbid` matches it. A `forbid` always wins, including over access that a role grants.

Access policies use their own action vocabulary. A policy names an entity type and a verb, such as `Action::"Agent.get"`, not the `dataplane_adp_*` permission strings listed on this page. See xref:control:access-policies.adoc#action-reference[Action reference].

When access policies aren't enabled for your organization, RBAC behaves exactly as this page describes, with no policy layer involved.
