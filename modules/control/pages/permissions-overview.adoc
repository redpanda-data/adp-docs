...
As a preview capability, you can layer xref:control:access-policies.adoc[access policies] on top of RBAC. Access policies are Cedar-based `permit` and `forbid` statements that express what roles can't: conditions over resource tags and ownership, grants scoped to a single resource, and cross-cutting denials that override every role.
