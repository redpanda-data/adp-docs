...
* Overrides are applied invisibly: `SpendingService` responses carry the resulting dollar values with no marker for which line items used an overridden rate.

// Verified 2026-06-09 against spending_service.proto and spending_rollup.proto on cloudv2 origin/main: no override flag or indicator field exists on breakdown or time-series responses.
