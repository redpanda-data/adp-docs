...
Adding an override for an agent that the tenant default budget was already governing takes over the agent's current-period spend rather than starting it fresh: the spend the agent has already accrued this period continues to count toward the override's limit, and the period keeps resetting on its normal schedule. Deleting and recreating an override behaves the same way, so an agent can't gain fresh runway by having its override recreated mid-period. An override created for an agent with no tenant default in effect is that agent's first budget, so it counts spend only from the time you create it. Editing an existing budget (for example, raising or lowering its limit) never moves the spend window; spend already accrued this period keeps counting.
