...
:learning-objective-3: Locate where to view spend in the Cost & Usage tab, in transcripts, and through breakdown queries

The Agentic Data Plane caps LLM spend with budgets and records every LLM call as a spending event. Set a budget to enforce a hard spending cap per agent, then read what you actually spend through the *Cost & Usage* tab on *LLM Providers*, through individual glossterm:transcript[,transcripts], and through breakdown queries by provider, model, user, agent, or provider type.
...
You don't view spend on this page. The *LLM Providers* page, transcripts, and breakdown queries are the read surfaces:
...
`SpendingService.GetSpendingBreakdown` is the canonical RPC for pulling spend out of ADP. Use it for chargeback reporting, scheduled emails, internal cost dashboards, or any workflow the built-in UI doesn't cover. Every spend number the UI shows comes from this RPC, so query results match the UI to the cent.
...
The `SpendingFilter` exposes `organization_id` and `user_email`, so every cost and usage query and every API call can scope to a single tenant or user. Use this to:

* See per-tenant spend in the *Cost & Usage* tab's provider breakdown.
...
The Agentic Data Plane ships with a default rate card per provider, covering input, output, and cache-read prices for every model in the built-in catalog. The cost-reporting pipeline reads from this rate card when it computes per-call spend, which is why every dollar value in the *Cost & Usage* tab, in transcripts, and in `SpendingService` queries works without any setup.
...
// TODO: confirm the exact UI route on `adp-production` once walked. Likely under *LLM Providers > \{provider} > \{model}*; capture the route + screenshots before merging.
...
* Overrides apply to *new spending events* from the moment they're saved. Historical events keep the rate card that was in effect when the event was recorded. Comparisons against a previous period reflect that.
...
* Cache-read pricing follows the catalog default. Only cache-write tiers are overridable. Cache-read tokens still surface in the `CACHED` bucket in the *Cost & Usage* tab and in `SpendingService` responses.
