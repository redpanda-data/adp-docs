...
The Agentic Data Plane caps LLM spend with budgets and records every LLM call as a spending event. Set a budget to enforce a hard spending cap per agent, then read what you actually spend through the glossterm:governance dashboard[], through individual glossterm:transcript[,transcripts], and through breakdown queries by provider, model, user, agent, or provider type.
...
Redpanda ADP enforces budgets per agent. In a budget, an _agent_ is identified by its resource name in the form `agents/<slug>`, the same identity that appears as `agent_name` in spend data. Because a budget keys on this resource name, deleting an agent and recreating it with the same name keeps it under the same budget.
...
Events flow through a Kafka pipeline and roll up into queryable storage. No setup required: the gateway captures spending the moment your first agent or MCP server runs through it.
...
AWS Bedrock evaluates guardrails and bills guardrail usage to the AWS account whose credentials the guardrail's backend uses. This cost is separate from ADP's LLM spend tracking: it does not appear in the spend view or `SpendingService`, which account only for LLM provider token usage through the gateway.

Bedrock charges for guardrail evaluation whether or not it blocks a request. Blocking an input avoids the downstream model-inference charge; blocking an output still incurs inference up to the point of generation. Bedrock doesn't charge for deterministic checks such as word filters and custom regex patterns, but it does charge for model-based policies such as content filters, denied topics, and contextual grounding, so turn off the directions and policies you don't need. For current rates, see the https://aws.amazon.com/bedrock/pricing/[AWS Bedrock pricing^].
...
For each model, the editor exposes buckets that match the cost-reporting categories:
...
|`Cache write`
|Cost per 1M tokens written to a prompt cache. Split into TTL tiers because providers charge differently per tier:
...
// TODO: roadmap items not yet shipped (keep in sync with the NOTE under "Manage budgets through the API"): tenant-wide shared pools; limits scoped by model, provider, or organization; and notification delivery (email, webhook, or chat) on warn or block. Document each here once it ships. Open Q C1 in the companion plan.
