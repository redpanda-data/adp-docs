...
AWS bills guardrail evaluation directly to the AWS account whose credentials the guardrail's backend uses. This cost does not appear in ADP cost reporting and is not counted against budgets. For current rates, see https://aws.amazon.com/bedrock/pricing/[AWS Bedrock pricing^].
...
The Agentic Data Plane ships with default per-model pricing per provider, covering input, output, and cache-read prices for every model in the built-in catalog. Cost reporting uses these prices when it computes per-call spend, which is why every dollar value in the *Cost & Usage* tab, in transcripts, and in `SpendingService` queries works without any setup.

If your organization negotiates non-standard pricing, or you want to track spend against an internal chargeback rate, override the rates as part of configuring an LLM provider. Overrides are scoped to a single provider, where you edit the rate per model. See xref:gateway:configure-provider.adoc[Configure an LLM provider].
