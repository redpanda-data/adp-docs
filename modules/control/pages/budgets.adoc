...
[NOTE]
====
Budgets currently cap spend per agent. Tenant-wide shared pools and limits scoped by model, provider, or organization are planned but not yet available.
====
...
// TODO: roadmap items not yet shipped (keep in sync with the NOTE under "Manage budgets through the API"): tenant-wide shared pools; limits scoped by model, provider, or organization. Document each here once it ships. Open Q C1 in the companion plan.
