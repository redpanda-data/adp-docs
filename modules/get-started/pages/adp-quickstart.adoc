...
:description: Set up Redpanda ADP end to end. As an administrator, configure your first LLM provider for the organization. As a builder, stand up an MCP server backed by a public OpenAPI spec and build a working agent that calls both.
...
* An *administrator* configures an LLM provider for your organization. This is a one-time setup task. Until a provider exists, no one can build agents against it.
...
A provider is an organization-level resource. Configure it once, and every builder can select it when they create agents. The next three sections walk through getting an Anthropic API key, storing it in the secret store, and pointing ADP at Anthropic.
...
Combine the Anthropic provider and the PetStore MCP server into an agent. ADP walks you through a managed agent in a six-step wizard: Basics, Model, Behavior, Tools, Subagents, and Review. For the full wizard reference, see xref:connect:create-agent.adoc[Create an agent].
...
+
* *LLM provider*: Select `anthropic-quickstart`. This provider supplies the credentials and routing for the agent. The dropdown lists every provider configured for your organization. If it's empty, no provider exists yet: configure one as shown earlier, or ask your administrator to configure one.
