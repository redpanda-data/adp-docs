...
This quickstart shows how to set up Redpanda ADP for your organization. You'll create an Anthropic API key, configure that key as an LLM provider, stand up an MCP server backed by a public OpenAPI spec, and build an agent that uses both.
...
Combine the Anthropic provider and the PetStore MCP server into an agent. ADP walks you through a managed agent in a step-by-step wizard.
...
+
* *Display name*: `pet-store-assistant`. This name appears in the registry list and the detail header.
...
+
* *LLM provider*: Select `anthropic-quickstart`. This provider supplies the credentials and routing for the agent.
