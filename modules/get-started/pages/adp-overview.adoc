...
ADP is its own product surface. It runs on Redpanda, so there's no separate cluster to stand up first, and at launch it deploys in your own AWS account.
...
The glossterm:governance dashboard[] gives platform administrators and finance partners one cross-tenant view of agent activity, spending, MCP server inventory, and authorization events, whether the agents and servers are Redpanda-hosted or your own. From a single overview, you can answer how much your organization spends on AI and what agents run in your environment, down to per-provider spending, agent state, and the authorization decisions that gate every tool invocation.
