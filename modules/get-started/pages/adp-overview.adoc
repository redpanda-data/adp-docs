...
:learning-objective-1: Identify the key components of Redpanda Agentic Data Plane and their purposes
:learning-objective-2: Describe how each component addresses enterprise governance and reliability requirements
:learning-objective-3: Determine whether Redpanda Agentic Data Plane fits your organization's requirements for AI agent deployment
...
.Runtime ownership changes, but gateway identity and governance stay consistent
image::resource-ownership-models.svg[alt="Resource ownership comparison. Redpanda runs managed agent runtimes and managed MCP server implementations. Customers run self-managed agent runtimes and self-managed MCP endpoints. AI Gateway and MCP Gateway provide identity, policy, and observability across both ownership models. LLM providers remain upstream model services connected through an Agentic Data Plane provider configuration."]
...
.Redpanda Agentic Data Plane sits between your agents and clients and your external systems and LLM providers
image::adp-architecture.png[Architecture diagram: Redpanda-managed agents, bring-your-own agents over A2A, and Microsoft Teams connect to the Agentic Data Plane. Inside Agentic Data Plane, out-of-band policy applies identity, guardrails, and spend caps; the MCP Gateway provides governed tool and data access to systems of record and MCP servers; and an LLM provider gateway gives one entry point per provider to Anthropic, OpenAI, Gemini, Bedrock, and OpenAI-compatible models. Transcripts stream to the Redpanda Data Platform as OpenTelemetry traces for replay, audit, and cost reporting.]
