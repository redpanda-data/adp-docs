...
image::resource-ownership-models.svg[alt="Resource ownership comparison. Redpanda runs managed agent runtimes and managed MCP server implementations. Customers run self-managed agent runtimes and self-managed MCP endpoints. AI Gateway and MCP Gateway provide identity, policy, secrets, and observability for both. LLM providers remain upstream model services connected through an Agentic Data Plane provider configuration."]
