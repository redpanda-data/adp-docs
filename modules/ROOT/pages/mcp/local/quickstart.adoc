...
In this quickstart, you'll get your Claude AI agent talking to Redpanda Cloud using the xref:mcp:local/overview.adoc[Redpanda Cloud Management MCP Server].

If you're trying to deploy your own MCP server as a managed service inside your cluster, see xref:mcp:remote/quickstart.adoc[].
...
* At least version 25.2.3 of xref:redpanda-cloud:manage:rpk/rpk-install.adoc[`rpk` installed on your computer]
...
+
TIP: For other clients, see xref:mcp:local/configuration.adoc[].
...
NOTE: The MCP server does *not* expose API endpoints that result in delete operations by default. Use `--allow-delete` only if you intentionally want to enable delete operations. See xref:mcp:local/configuration.adoc#enable_delete_operations[Enable delete operations].

:note-caption: Note

== Next steps

* xref:mcp:local/configuration.adoc[]
