...
After xref:mcp:remote/quickstart.adoc[deploying your first MCP server], create custom tools that AI clients can discover and invoke. This guide walks you through the process using any Redpanda Connect component.
...
* You can describe the MCP execution model (see xref:mcp:remote/concepts.adoc#execution-model[The MCP execution model])
* You have chosen the right component type for your use case (see xref:mcp:remote/concepts.adoc#component-selection[Choose the right component type])
...
See xref:mcp:remote/tool-patterns.adoc#outputs-with-processors[outputs with processors] for more examples.
...
See xref:mcp:remote/best-practices.adoc#secrets[Secrets management] for more guidance.
...
* xref:mcp:remote/tool-patterns.adoc[]
