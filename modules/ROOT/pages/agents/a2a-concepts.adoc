...
To configure the agent card, see xref:agents/create-agent.adoc#configure-a2a-discovery-metadata-optional[Configure A2A discovery metadata].
...
For integration pattern guidance, see xref:agents/integration-overview.adoc[].
...
The `a2a_message` processor uses the A2A protocol internally to discover and call agents. For pipeline patterns, see xref:agents/pipeline-integration-patterns.adoc[].
