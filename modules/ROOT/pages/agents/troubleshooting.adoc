...
* Test agents with the xref:agents/prompt-best-practices.adoc#evaluation-and-testing[systematic testing approach]
...
* Follow tool design patterns in xref:mcp/remote/tool-patterns.adoc[]
...
* Set max iterations appropriate for task complexity (see xref:agents/concepts.adoc#why-iterations-matter[Why iterations matter])
...
* See cost calculation guidance in xref:agents/concepts.adoc#cost-calculation[Cost calculation]
...
* See xref:agents/a2a-concepts.adoc#agent-card-location[Agent card location] for details
...
* See integration patterns in xref:agents/pipeline-integration-patterns.adoc[]

== Monitor and debug agents

For comprehensive guidance on monitoring agent activity, analyzing conversation history, tracking token usage, and debugging issues, see xref:agents/monitor-agents.adoc[].
