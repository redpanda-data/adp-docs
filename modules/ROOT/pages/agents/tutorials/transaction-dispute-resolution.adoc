...
* Completed xref:agents/tutorials/customer-support-agent.adoc[] (foundational multi-tool concepts).
...
[NOTE]
====
This tutorial uses XSmall resource tier for all MCP servers because the mock tools run lightweight Bloblang transformations. Production deployments with external API calls require larger tiers based on throughput needs. See xref:mcp/remote/scale-resources.adoc[].
====
...
For comprehensive troubleshooting guidance, see xref:agents/troubleshooting.adoc[].
...
* xref:agents/monitor-agents.adoc[]
