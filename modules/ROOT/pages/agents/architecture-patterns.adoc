...
Avoid these architecture mistakes that lead to unmaintainable agent systems. For examples of well-structured agents, see the xref:agents/tutorials/customer-support-agent.adoc[multi-tool orchestration tutorial] and xref:agents/tutorials/transaction-dispute-resolution.adoc[multi-agent architecture tutorial].
...
Limit tools per agent to 10-15 for optimal performance. Agents with more than 20-25 tools often show degraded tool selection accuracy. Use subagents to partition tools by domain. For tool design patterns, see xref:mcp/remote/tool-patterns.adoc[].
