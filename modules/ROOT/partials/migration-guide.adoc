...
....
flowchart TB
    App["Application"] --> Flag["Feature flag"]
    Flag --> Direct["Direct provider"]
    Flag --> Gateway["Gateway route"]
....
...
1. Check geographic distance (gateway → provider region)
2. Verify provider pool configuration (no unnecessary fallbacks)
3. Check for rate limiting (adds retry latency)

Solution: Review gateway region and provider pool configuration.
...
* Gradual rollout through provider pool configuration
