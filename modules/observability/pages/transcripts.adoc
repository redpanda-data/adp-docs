...
For conceptual background on spans and trace structure, see xref:concepts.adoc[].
...
To view nested operations, expand any parent span. To learn more about span hierarchies and cross-service traces, see xref:concepts.adoc[].
...
* **Attributes tab**: Structured metadata for debugging (see xref:concepts.adoc#key-attributes-by-layer[standard attributes by layer]).
