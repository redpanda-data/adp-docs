...
* xref:agents:create-agent.adoc[Running agent] or xref:mcp:create-server.adoc[MCP server] with at least one execution
...
* The model is in the pricing table. To use a custom rate (negotiated contract, internal chargeback), see xref:governance:budgets.adoc#override-per-model-pricing[Override per-model pricing].
...
* xref:agents:troubleshoot/troubleshoot-ai-agents.adoc[]
